...
for (alt in alt_names) {
  ...
  row_idx     <- row_map[[alt]]
  # The bootstrap fits are block=TRUE, so the observed Lambda must also use the
  # BLOCK fit (block_ll/block_AIC) for the unit-specific alternative, otherwise
  # observed-vs-bootstrap is mismatched. (Mirrors the corrected SIRJPF2 reference.)
  alt_ll_obs  <- target_no_para_parameter_table[row_idx, "block_ll"]
  alt_AIC_obs <- target_no_para_parameter_table[row_idx, "block_AIC"]
