...
REQUIRES the development pypomp tree (with PanelPomp/PanelParameters/RWSigma/ParTrans);
the older PyPI `pypomp 0.0.4` lacks this panel API and the import will fail (loudly, not
silently). Put the dev pypomp on PYTHONPATH on the GPU host.
...
What this script reproduces EXACTLY is the *estimator and workflow*:
  ...
  * the identical profile design -- generated by calling R's seeded
    pomp::profile_design through a subprocess, with the SAME seed formula
    coverage_profile.R uses (805*1000 + 1000*pidx + b),
...
# ----------------------------------------------------------------------------
# 3. R-subprocess bridge: read .rds, run pomp::profile_design, write .rds
#    (kept so the profile design is reproduced EXACTLY from R's seeded RNG)
# ----------------------------------------------------------------------------
def run_rscript(code: str, args: list[str]) -> str:
    completed = subprocess.run(
        ["Rscript", "--vanilla", "-e", code, *args],
...
def read_sim_data_rds(path: Path) -> dict[str, pd.DataFrame]:
    """Read simulated_data/sim_data_<b>.rds (list u1..u8 of day,4-obs matrices)."""
    code = r"""
    ...
    for unit_name in UNIT_NAMES:
        unit_df = pd.DataFrame(parsed[unit_name])
...
def read_params_rds(path: Path) -> dict[str, float]:
    """Read simulated_data/true_params.rds (named numeric vector of 26)."""
    code = r"""
    args <- commandArgs(trailingOnly = TRUE)
    x <- readRDS(args[1])
    if (!requireNamespace("jsonlite", quietly = TRUE)) stop("R pkg jsonlite required")
...
def generate_parameter_profile(params_path: Path, dataset_index: int,
                               prof_name: str, nprof: int = 80) -> pd.DataFrame:
    """
    Reproduce coverage_profile.R's profile design EXACTLY via R's seeded
    pomp::profile_design.  Seed formula matches coverage_profile.R line 44:
        set.seed(805*1000 + 1000*pidx + b),  pidx = match(prof_name, profilable).
    """
    pidx = PROFILABLE.index(prof_name) + 1            # R match() is 1-based
    seed = 805 * 1000 + 1000 * pidx + dataset_index
    code = r"""
    args <- commandArgs(trailingOnly = TRUE)
    true_params <- readRDS(args[1])
    prof_name   <- args[2]
    nprof       <- as.integer(args[3])
    seed        <- as.integer(args[4])
    if (!requireNamespace("jsonlite", quietly = TRUE)) stop("R pkg jsonlite required")
    if (!requireNamespace("pomp", quietly = TRUE))     stop("R pkg pomp required")
    set.seed(seed)                                      # [R: coverage_profile.R line 44]
    shared_ub <- true_params * 10                       # [R: lines 184-191]
    ...
    parameter_shared <- pomp::profile_design(
      ...
      lower = log(shared_lb), upper = log(shared_ub),
      type  = "runif", nprof = nprof
    )
    names(parameter_shared)[names(parameter_shared) == "temp"] <- prof_name
    parameter_shared <- exp(parameter_shared)
    parameter_shared$sigSn <- 0                         # [R: lines 202-203]
    parameter_shared$sigSi <- 0
    cat(jsonlite::toJSON(parameter_shared, dataframe = "rows", auto_unbox = TRUE, digits = 17))
    """
    rows = json.loads(run_rscript(code, [str(params_path), prof_name, str(nprof), str(seed)]))
    profile_df = pd.DataFrame(rows)
    missing = [n for n in PARAM_NAMES if n not in profile_df.columns]
    if missing:
        raise ValueError(f"profile design missing parameters: {missing}")
    return profile_df[PARAM_NAMES]                       # canonical column order


def write_profile_rds(profile_df: pd.DataFrame, path: Path) -> None:
    """Write the profile output as an .rds tibble (same shape collect_coverage.R reads)."""
...
def main(argv: list[str]) -> int:
    ...
    parameter_shared = generate_parameter_profile(params_path, b, name_str, args.nprof)
