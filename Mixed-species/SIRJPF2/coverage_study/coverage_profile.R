...
if (length(args) < 2) {
  stop("Usage: ./coverage_profile.R <dataset_index> <parameter>\n  e.g. ./coverage_profile.R 42 rn")
}
b <- as.integer(args[1])
...
## Parameters that can be profiled; index also fixes a reproducible per-parameter
## seed offset so (parameter, dataset) pairs never share a seed.
profilable <- c("xi","theta_Sn","theta_Si","theta_In","theta_Ii","theta_P","theta_Jn","theta_Ji",
                "rn","ri","f_Sn","f_Si","probn","probi","sigIn","sigIi","sigJn","sigJi",
                "sigF","sigP","k_Sn","k_Si","k_In","k_Ii")
pidx <- match(name_str, profilable)
if (is.na(pidx)) {
  stop("Unknown profile parameter: ", name_str,
       ".\n  Valid options: ", paste(profilable, collapse = ", "))
}

cat("=== Coverage profile", name_str, ": dataset", b, "===\n")
...
## Random seed: reproducible, distinct per (parameter, dataset)
set.seed(805 * 1000 + 1000 * pidx + b)
...
## ---------------------------------------------------------------------------
## 4. Profile design for rn (same structure as profile_rn.R)
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 8. Extract subset_data (rn + loglik only) — the minimal data for mcap()
## ---------------------------------------------------------------------------
...
# Group by the profiled parameter, keep the best loglik at each profile point.
# Keep ALL coefficient columns so composite targets (e.g. ri*f_Si, probi*f_Si)
# can be reconstructed in collect_coverage.R.
subset_data <- final_params %>%
  group_by(.data[[name_str]]) %>%
  filter(loglik == max(loglik)) %>%
  ungroup()

# Add log-transformed profiled parameter (needed by mcap)
subset_data[[paste0("log_", name_str)]] <- log(subset_data[[name_str]])
...
out_file <- sprintf("coverage_results/profile_%s_%03d.rds", name_str, b)
saveRDS(subset_data, file = out_file)

cat("Saved profile for", name_str, "dataset", b, "\n")
