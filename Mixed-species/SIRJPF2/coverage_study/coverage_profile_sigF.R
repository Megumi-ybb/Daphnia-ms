...
if (length(args) < 1) {
  stop("Usage: ./coverage_profile_sigF.R <dataset_index>\n  e.g. ./coverage_profile_sigF.R 42")
}
b <- as.integer(args[1])
...
cat("=== Coverage profile sigF: dataset", b, "===\n")
...
## ---------------------------------------------------------------------------
## 4. Profile design for sigF (same structure as profile_sigF.R)
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 8. Extract subset_data (sigF + loglik only) — the minimal data for mcap()
## ---------------------------------------------------------------------------
...
# Group by sigF, keep only the best loglik at each profile point
...
# Add log-transformed sigF (needed by mcap)
...
saveRDS(
  subset_data_sigF,
  file = sprintf("coverage_results/profile_sigF_%03d.rds", b)
)

cat("Saved subset_data_sigF for dataset", b, "\n")
cat("  Rows:", nrow(subset_data_sigF), "\n")
cat("  File size:", file.size(sprintf("coverage_results/profile_sigF_%03d.rds", b)), "bytes\n")
