...
if (length(args) < 1) {
  stop("Usage: ./coverage_profile_rn.R <dataset_index>\n  e.g. ./coverage_profile_rn.R 42")
}
b <- as.integer(args[1])
...
cat("=== Coverage profile rn: dataset", b, "===\n")
...
# Group by rn, keep only the best loglik at each profile point
...
# Add log-transformed rn (needed by mcap)
...
saveRDS(
  subset_data_rn,
  file = sprintf("coverage_results/profile_rn_%03d.rds", b)
)

cat("Saved subset_data_rn for dataset", b, "\n")
cat("  Rows:", nrow(subset_data_rn), "\n")
cat("  File size:", file.size(sprintf("coverage_results/profile_rn_%03d.rds", b)), "bytes\n")
