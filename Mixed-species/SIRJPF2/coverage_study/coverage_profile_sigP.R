...
if (length(args) < 1) {
  stop("Usage: ./coverage_profile_sigP.R <dataset_index>\n  e.g. ./coverage_profile_sigP.R 42")
}
b <- as.integer(args[1])
...
cat("=== Coverage profile sigP: dataset", b, "===\n")
...
## ---------------------------------------------------------------------------
## 4. Profile design for sigP (same structure as profile_sigP.R)
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 8. Extract subset_data (sigP + loglik only) — the minimal data for mcap()
## ---------------------------------------------------------------------------
...
# Group by sigP, keep only the best loglik at each profile point
...
# Add log-transformed sigP (needed by mcap)
...
saveRDS(
  subset_data_sigP,
  file = sprintf("coverage_results/profile_sigP_%03d.rds", b)
)

cat("Saved subset_data_sigP for dataset", b, "\n")
cat("  Rows:", nrow(subset_data_sigP), "\n")
cat("  File size:", file.size(sprintf("coverage_results/profile_sigP_%03d.rds", b)), "bytes\n")
