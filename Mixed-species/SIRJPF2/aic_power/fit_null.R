...
cat("AIC sim null fit: dataset", b, "truth =", truth, "\n")

# Seed structure: 802 * 1000 + b + truth_offset (0/500) + script_offset (0 for null)
truth_offset <- if (truth == "shared") 0 else 500
set.seed(802 * 1000 + b + truth_offset)
...
if (truth == "shared") {
  ...
  # Use the shared params + the mean of the unit-specific theta_In as the warm start
...
# ---- Model specification (same as bootstrap_lrt/fit_null.R) ----
dyn_rpro <- Csnippet("
...
panelfood <- panelPomp(pomplist, shared = parameters)

# ---- Algorithmic parameters (matches bootstrap_lrt) ----
Np <- 1000
Np_rep <- 10
Mp <- 1000
...
# actually perturbed by mif2, i.e. excludes sigSn and sigSi which are pinned at
# the boundary value 0 by rw.sd = 0. Of the 26 entries in param_names, 24 are
# free in the all-shared model.
k_null <- 24
AIC_null <- 2 * k_null - 2 * ll_best
...
saveRDS(result, file = sprintf("results/fit_null_%s_%d.rds", truth, b))
cat("Done. ll =", ll_best, "se =", se_best, "k =", k_null, "AIC =", AIC_null, "\n")
