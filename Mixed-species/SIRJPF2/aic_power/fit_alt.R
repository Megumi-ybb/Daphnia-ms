...
b <- as.integer(args[1])
truth <- args[2]   # "shared" or "unit_specific"
if (!(truth %in% c("shared", "unit_specific"))) {
  stop("truth must be 'shared' or 'unit_specific', got: ", truth)
}
cat("AIC sim alt fit (theta_In unit-specific): dataset", b, "truth =", truth, "\n")

# Seed structure: 802 * 1000 + b + truth_offset (0/500) + 100000 (alt script offset)
truth_offset <- if (truth == "shared") 0 else 500
set.seed(802 * 1000 + b + truth_offset + 100000)
...
specific_names <- c("theta_In")
cat("Unit-specific parameter:", paste(specific_names, collapse = ", "), "\n")

# ---- Model specification (same as bootstrap_lrt/fit_alt.R) ----
dyn_rpro <- Csnippet("
...
# ---- Build panelPomp with unit-specific theta_In ----
...
ll_best <- unname(mf[[best]]$ll[1])
se_best <- unname(mf[[best]]$ll[2])
# Project convention (matches lrt_all_summary.txt): k counts only the parameters
# actually perturbed by mif2. Null has k = 24 (excludes boundary-fixed sigSn,
# sigSi). Alt makes theta_In unit-specific: removes 1 shared param, adds 8
# unit-specific ones, net +7 over the null, giving k_alt = 31.
k_alt <- 24 - length(specific_names) + length(specific_names) * 8
AIC_alt <- 2 * k_alt - 2 * ll_best
...
saveRDS(result, file = sprintf("results/fit_alt_%s_%d.rds", truth, b))
cat("Done. ll =", ll_best, "se =", se_best, "k =", k_alt, "AIC =", AIC_alt, "\n")
