...
\begin{figure}[ht]
\centering
<<DataVis,echo=FALSE, fig.height=3, cache=cache_option>>=
...
The top panel shows adult female susceptibles (\textit{D.~dentifera}, blue; \textit{D.~lumholtzi}, black) and infecteds (\textit{D.~dentifera}, red; \textit{D.~lumholtzi}, purple).
