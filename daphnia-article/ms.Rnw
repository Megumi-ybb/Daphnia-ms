...
This ability was unlocked by panel iterated filtering \citep{breto20} and further strengthened by the marginalized panel iterated filtering algorithm that we develop and demonstrate here \jwc{Should we discuss the relationship between this paper and the MPIF methods paper? Both are coming out at roughly the same time, should we put one (or both) on ArXiv to be able to cite it?}.
...
This led us to introduce a latent food compartment, and we found that including age structure via a juvenile stage also helps to improve the statistical fit of the model.
...
Our hypothesized model is supported so far as it is the best available model according to this criterion; our model and data are available at \href{https://github.com/Megumi-ybb/Daphnia-ms}{github} to facilitate further investigations \jwc{Is including a link the appropriate thing here? I think making a Zenodo (or similar) repository, and just mentioning their existance and including more details at the end of the document.}
...
The subscript $u$ ranges in $\seq{1}{U}$ with $U=8,9,10$ depending on the specific treatment \jwc{Is the treatment not fixed for the SIRPF2 model?}.
...
Latent state estimation and parameter inference \jwc{In what way for parameter inference, isn't this difficult enough to require IF2 rather than just the PF?} in POMP models is commonly carried out using the particle filter, also known as sequential Monte Carlo \citep{arulampalam02}.
...
\jwc{I made some larger edits to the next two paragraphs.}
...
\jwc{This is a fairly long paragraph, let's consider shortening it or breaking it up.}
