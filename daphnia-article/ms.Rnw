...
We also consider several variants of the SIRJPF2 model described here, namely a model that excludes the juvenile state (SIRPF2, described in Section~{\suppSecBenchmark}.3)\eic{THIS COULD BE IN ITS OWN SECTION. IT IS AN ABLATION STUDY, REMOVING J, COMPARABLE TO THE F ABLATION STUDY}, and a version of the SIRJPF2 model where the Gaussian noise has been replaced with Gamma noise (SIRJPF2-Gamma, see Section~{\suppSecSIRJPFgamma}).
