...
At the same time, standard Bayesian methods cannot accommodate the nonlinear, non-Gaussian mechanistic panel models considered here \citep{wigren22}, and no ecological study has yet achieved full-likelihood inference for them. 
