...
In response to adverse environmental cues, \textit{Daphnia} produce males and females begin reproducing sexually, producing durable resting eggs called ephippia \citep{radzikowski18,searle16salinization}.
...
Differences in AIC values between models are intepretable only between alternative models for the same data, so we fit a few statistical benchmarks to help calibrate these results.
...
Because the improvements to model fits evidence the observed data more accurately, it more faithfully captures the underlying ecological mechanisms, which thereby uncovering new scientific insights.
...
And the invasive species has a lower rate of infection per consumed spore ($p^{\native} < p^{\invasive}$), consistent with Fig.~1C of \citet{searle16}.
...
This asymmetry is attributable to (i) the lower density of \textit{D. lumholtzi} allows less potential data collection error tolerance; (ii) the product of these terms governs the juvenile growth and infection process, which allows individual invasive coefficients to vary so long as their products remain fixed; and (iii) host-only experiments with simplified dynamics (Fig.~S8) in which these parameters become fully identifiable, confirming that their limited identifiability in the full model is data–, not model–, driven.
However, we achieve the identifiability of the effective infection rate ($p^{\native}f_{S^{\native}}$, $p^{\invasive}f_{S^{\invasive}}$), and the effective juvenile birth rate ($r^{\native}f_{S^{\native}}$, $r^{\invasive}f_{S^{\invasive}}$), which support our model's scientific interpretability.
