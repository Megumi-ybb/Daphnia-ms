...
\node (M) [decision2, left of=U, xshift=-0.5cm] {\texttt{m=1:M}};
\node (S) [mytitle, above of=M, xshift=-0.37cm, yshift = -2.2cm] {\textbf{Input:} PanelPOMP model\\ and data};

\draw[arrow,black] (initialize1) -- (N);
\draw[arrow,black] (U) -- (initialize1);
\draw[arrow,black] (M) -- (U);
\draw[arrow,black] (S) -- (M);
...
\node (O) [mytitle, left of=resampleProcess, xshift=-15.0cm]
