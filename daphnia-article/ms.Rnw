...
\begin{frontmatter}
\title{Mechanistic models for panel data: Analysis of ecological experiments with four interacting species}
...
This study applies iterated particle filtering techniques to explore new possibilities for likelihood-based statistical analysis of these complex systems.
...
Time series data were collected on replicated mesocosms under six treatment conditions.
...
%% Our iterated filtering framework provides a statistically efficient, likelihood-based solution, avoiding the information loss inherent when using the generalized method of moments or Approximate Bayesian Computing approaches.
%% Existing full-information Bayesian tools are not applicable to panels of partially observed Markov process models with intractable transition densities.
{\revision\editAbstractScience}
\end{abstract}

\begin{keyword}
\kwd{Panel Iterated Filtering}
\kwd{SIRJPF model}
\kwd{PanelPOMP}
\kwd{Daphnia}
\end{keyword}
...
Partially observed Markov process (POMP) models, also known as hidden Markov models or state space models \citep{breto09,auger-methe21}, comprise a class of models suited to the description of such systems, which describe the latent process using a Markovian representation.
 ...
 \revision{\editMesocosm}
  ...
  For either of these goals, investigation of panel data leads to the challenging statistical task of fitting a collection of related nonlinear or non-Gaussian vector-valued POMP models, called a PanelPOMP model \citep{breto20,breto25}.
...
\revision{\editBayes}
 ...
 Our goal is to ensure that the proposed dynamic model provides a plausible mechanistic description of the dynamic system, supported by evidence that the model also provides a suitable quantitative description of data that arise from the system.
...
This ability was unlocked by panel iterated filtering \citep{breto20} and we demonstrate empirically that this is further strengthened by a new marginalized panel iterated filtering algorithm.
...
\revision{\editParthenogenesis}
Maturation of juveniles to adults takes approximately five to ten days, depending on environmental conditions.
In response to adverse environmental cues, \textit{Daphnia} produce males, and females reproduce sexually to produce durable resting eggs called ephippia \citep{radzikowski18,searle16salinization}.
...
\begin{figure}[ht]
\centering
<<DataVis,echo=FALSE, fig.height=4.2, cache=cache_option>>=
...
library(ggplot2)
# Following the AOAS grayscale request, line type carries the species signal
# and colour is a secondary cue. dent = solid blue; lum = dashed black.
lum.style    <- "twodash"
dent.style   <- "solid"
lum.width    <- 0.6
dent.width   <- 0.5
...
# Row 1: adult susceptible females
p_adultS <- ggplot(mixed_para_data_combined, aes(x = day)) +
  geom_line(aes(y = dent.adult), color = dent.color, linewidth = dent.width,
            linetype = dent.style) +
  geom_line(aes(y = lum.adult),  color = lum.color,  linewidth = lum.width,
            linetype = lum.style) +
  ...
  ylab('Adult susceptible') +
  scale_x_continuous(limits = c(0, 52), breaks = c(0, 25, 50)) +
  scale_y_continuous(trans = "sqrt")

# Row 2: adult infected females
p_adultI <- ggplot(mixed_para_data_combined, aes(x = day)) +
  geom_line(aes(y = dent.inf),     color = dent.color, linewidth = dent.width,
            linetype = dent.style) +
  geom_line(aes(y = lum.adult.inf), color = lum.color, linewidth = lum.width,
            linetype = lum.style) +
...
# Row 3: juveniles (susceptible only -- negligible infected juveniles)
p_juv <- ggplot(mixed_para_data_combined, aes(x = day)) +
  geom_line(aes(y = dent.juv), color = dent.color, linewidth = dent.width,
            linetype = dent.style) +
  geom_line(aes(y = lum.juv),  color = lum.color,  linewidth = lum.width,
            linetype = lum.style) +
  facet_wrap(~Bucket, nrow = 1) +
  theme_minimal() +
  theme(
...
grid.arrange(p_adultS, p_adultI, p_juv, ncol = 1)
@
\caption{\label{fig:data_vis}
The experimental data.
Density (individuals per litre) of adult susceptible females (top), adult infected females (middle) and juveniles (bottom).
\textit{D.~dentifera} is plotted with solid lines (blue in the color version), and \textit{D.~lumholtzi} with dashed lines (black in the color version).
There were negligible infected juveniles and males.
Each column corresponds to one mesocosm replicate.
}
\end{figure}
...
The simpler treatments are presented in Supplementary Sections~S7,~S8 and~S9.
...
The males arise predominantly later in the experiment as shown in Table S14.
...
We also hypothesize that the inclusion of age structure, via a juvenile stage, could help to explain the population dynamics even though very few juveniles show disease.
...
Our hypothesized model is supported so far as it is the best model for the available data according to this criterion.
...
\node (In) at (5.5,-1.5) [square] {$I^\native$};
\node (Si) at (-0.5,0) [square] {$S^\invasive$};
\node (Ii) at (-0.5,-1.5) [square] {$I^\invasive$};
\node (Jn) at (5.5,1.5) [square] {$J^\native$};
\node (Ji) at (-0.5,1.5) [square] {$J^\invasive$};
\node (R) at (2.5,0)  [rhombus] {$R$};
\node (P) at (2.5,-1.5)[travel] {$P$};
\node (F) at (2.5,1.5) [report] {$F$};
...
\end{center}
\vspace{-5mm}
\caption{The SIRJPF2 model.
The two species of \textit{Daphnia} (square blue compartments) interact with each other indirectly through $F$ (Food) and $P$ (Parasite) compartments.
Transition to the $R$ state represents death or removal of any species.
}
\label{fig:flow_SIRJPF2}
\end{figure}

Fig.~\ref{fig:flow_SIRJPF2} shows a diagram showing interactions in a two-species model where the \textit{Daphnia} are susceptible adult female (S), infected adult female (I), or juvenile (J), with a superscript $\invasive$ for invasive and $\native$ for native.
\revision{\editMales}
We keep track of the aquatic parasite spore density (P) and the food density (F).
Removal (i.e., death, consumption or destructive sampling) for any species is described by transition to class R.
This dynamic system extends the basic SIR model fundamental to epidemiology \citep{weiss13}.
Mathematically, the SIRJPF2 model is described by equations~(\ref{eq:model:first}--\ref{eq:model:last}), a system of stochastic differential equations which we interpret in the It\^{o} sense \citep{oksendal98}.
...
A version of SIRJPF2 with gamma noise, called SIRJPF2-Gamma, is presented in Section~S12, though we found that a simpler Gaussian noise model is sufficient.
...
\editSubscript
Most parameters are written with potential dependence on $u$ to enable investigation of factors explaining differences between replicates.
...
\begin{align}
    ...
    dP_{\unit}(t) &= \hspace{-1mm} \sum_{\species \in \{\native, \invasive\}} \hspace{-1mm} \left(\beta^{\species}_{\unit} \, \theta^{\species}_{I,{\unit}} \,  I^{\species}_{\unit}(t) - f^{\species}_{S,{\unit}} \big\{ S^{\species}_{\unit}(t) + \xi_{\unit} I^{\species}_{\unit}(t) \big\} P_{\unit}(t)\right) \! dt
    \\ \nonumber & \hspace{5.5cm}
    - \theta_{P,{\unit}} \, P_{\unit}(t)\, dt + P_{\unit}(t)\, d\zeta_{P,{\unit}},\\
...
Simpler treatments with a single \textit{Dapnia} species (SIRJPF) and without the parasite (SRJF and SRJF2) arise as special cases of SIRJPF2, with appropriate states and parameters
\revision{\editSetToZero}
These experiment treatments and corresponding models are presented in Sections~S7, S8 and S9.
...
The plug-and-play property means that we never require explicit evaluation of this density, only the ability to obtain simulated trajectories satisfying equations~(\ref{eq:model:first}--\ref{eq:model:last}).
...
The panel extension of iterated filtering by \citet{breto20} applies the same principle to PanelPOMP models.
...
\node (initialize) [process, left of=N, xshift=-1cm] {Initialize particle \texttt{j}, unit \texttt{u},\\
time $\mathtt{t_0}$, states \& parameters};
\node[fit=(initialize),myfit] (initialize1) {};
\node[mytitle] at (initialize1.north west) {\texttt{j=1:J}};

\node (U) [decision, left of=initialize1, xshift=-1cm] {\texttt{u=1:U}};
\node (M) [decision2, left of=U, xshift=1.4cm] {\texttt{m=1:M}};
\node (S) [mytitle, above of=M, xshift=0.37cm, yshift = -1.8cm] {\textbf{Input:} PanelPOMP model\\ and data};

\draw[arrow,black] (initialize1) -- (N);
\draw[arrow,black] (U) -- (initialize1);
\draw[arrow,black] (M) -- (U);
\draw[arrow,black] (S)+(-136:1.0cm) -- (M);

\node (predict) [process, right of=N, xshift=1cm, yshift=0.1cm] {Simulate stochastic dynamics,\\ unit $\mathtt{u}$, time $\mathtt{t_{n-1}}$ to $\mathtt{t_{n}}$};
\node (perturb) [process2, above of=predict, yshift=-2.7cm] {Perturb parameters $\mathtt{\phi}$, $\mathtt{\psi_u}$};
\node (weight)  [process,  below of=predict, yshift=2.7cm]  {Calculate weights for data,\\ unit $\mathtt{u}$, time $\mathtt{t_{n}}$};
...
\node (resampleState)  [process,  below of=weight,        yshift=1cm] {Resample states};
\node (resampleParams) [process2, below of=resampleState, yshift=2.5cm] {Resample parameters};
\node[fit=(resampleState)(resampleParams),myfit] (resampleProcess) {};
\node[mytitle] at (resampleProcess.north west) {\texttt{j=1:J}};
\draw[arrow,black] (ppw.south) -- (resampleProcess.north);

\node (O) [mytitle, left of=resampleProcess, xshift=-12.75cm]
    {\textbf{Output:} Monte Carlo Maximum\\ Likelihood Parameter Estimate};
\draw[arrow,black] (resampleProcess.west) -- (O.east);

\coordinate (rightLane) at ([xshift=-0.5cm] resampleProcess.west);
\coordinate (loopbus)   at ([yshift=2cm] O.south);
\coordinate (busRight)  at (rightLane |- loopbus);
\draw[arrow,black] (resampleProcess.west) -- (rightLane) -- (busRight)
  -- (N.south |- loopbus) -- (N.south);
\draw[arrow,black] (resampleProcess.west) -- (rightLane) -- (busRight)
  -- (U.south |- loopbus) -- (U.south);
\draw[arrow,black] (resampleProcess.west) -- (rightLane) -- (busRight)
  -- (M.south |- loopbus) -- (M.south);
...
\bye{All likelihood maximization in this article uses the marginalized variant of panel iterated filtering (MPIF; Fig.~\ref{fig:pif}), rather than the panel iterated filter of \citet{breto20} in its original form. Both algorithms extend iterated filtering \citep{ionides15} to PanelPOMP models by associating a perturbed parameter value with each particle $j$ and using particle-filter resampling to concentrate the parameter swarm around the maximum likelihood estimate; they differ only in the parameter resampling step. When the swarm is filtered through the data of unit $u$, the algorithm of \citet{breto20} resamples not only the shared parameters $\phi$ and the focal unit's parameters $\psi_u$ but also the parameters $\psi_{\tilde u}$ of every other unit $\tilde u \neq u$, even though the data for unit $u$ are uninformative about $\psi_{\tilde u}$. The marginalized variant instead resamples only $\phi$ and $\psi_u$, leaving each $\psi_{\tilde u}$ unchanged while filtering through unit $u$. This respects the defining conditional-independence structure of a PanelPOMP, under which the latent processes are independent across units given $\phi$ (Section~\ref{sec:meth}), so that the resampling weights generated by a single unit do not perturb the parameter estimates of the others. We adopt MPIF here because it is what makes reliable likelihood maximization and unit-specific inference feasible for this eight-unit, weakly identified mechanistic panel: the eight mesocosms differ in species composition, density and parasite load, and under joint resampling the units that happen to fit poorly dominate the particle weights and degrade the diversity of the unit-specific parameter swarm. The marginalized variant is the block-resampling option of the \texttt{mif2} function in the \texttt{panelPomp} R package, and is specified in our implementation by setting $\mathrm{MARGINALIZE}=\mathrm{TRUE}$ in the algorithm of Section~S2; we found it to give substantially more reliable optimization than joint resampling for the model and data analyzed here. A contemporaneous theoretical and methodological treatment of marginalized panel iterated filtering, including its convergence properties, is provided by \citet{wheeler25}.}
...
Various other information criteria have been proposed and used in ecological data analysis, in addition to likelihood ratio hypothesis tests \citep{johnson04}.
...
To obtain confidence intervals, we correct for the Monte Carlo uncertainty in the profile likelihood evaluations by using the Monte Carlo Adjusted Profile (MCAP) method of \citet{ionides17}.
\bye{The finite-sample coverage properties of MCAP on the SIRJPF2 model have been validated by a parametric bootstrap simulation study, presented in Section~S3 of the supplement.}
...
k_Si = parms$k_Si
k_Sn = parms$k_Sn
probi_f_Si = parms$probi * parms$f_Si
probn_f_Sn = parms$probn * parms$f_Sn
ri_f_Si = parms$ri * parms$f_Si
rn_f_Sn = parms$rn * parms$f_Sn
lambda_Jn = 0.1
lambda_Ji = 0.1
beta_n = 30.00
beta_i = 30.00
...
load('../data/Target_dynamics/para/profile_ci_table.rda')
...
\hline
Notation & Definition &Unit & Value & CI\\[0.5ex]
\hline
\hline
$S^{\species}_{\unit}(t)$ & Susceptible host density for species $\species\in\{\native,\invasive\}$  & $\mathrm{individual} \cdot L ^{-1}$                 & Variable                & \\
$I^{\species}_{\unit}(t)$ & Infected host density for species $\species\in\{\native,\invasive\}$     & $ \mathrm{individual} \cdot L ^{-1}$                & Variable                & \\
$J^{\species}_{\unit}(t)$ & Juvenile host density for species $\species\in\{\native,\invasive\}$     & $ \mathrm{individual} \cdot L ^{-1}$                & Variable                & \\
$F_{\unit}(t)$   & Alga density                              & $10^6 \cdot \mathrm{cell} \cdot L ^{-1}$             & Variable                & \\
$P_{\unit}(t)$   & Spore density                             & $10^3 \cdot \mathrm{spore} \cdot L ^{-1}$            & Variable                & \\
...
$r^{\invasive}$ & Birth rate of invasive juvenile                       & $\mathrm{individual} \cdot 10^{-6}\cdot \mathrm{cell}^{-1}$ & \Sexpr{mysignif(ri,2)}  & (\Sexpr{mysignif(ci_table$ri[1],2)}, $\infty$)\\
...
$p^{\invasive}$          & Number of invasive infections per spore           &     $10^{-3} \cdot \mathrm{individual} \cdot \mathrm{spore}^{-1} $ & \Sexpr{mysignif(probi,2)}  & (\Sexpr{mysignif(ci_table$probi[1],2)},$\infty$)\\
...
$\sigma^{\invasive}_{I}$ & Standard deviation of Brownian motion of infected invasive adult       &   $\sqrt{\mathrm{individual} \cdot \mathrm{day}^{-1}}$  & \Sexpr{mysignif(sigIi,2)}  & (\Sexpr{0},$\infty$)\\
...
\end{tabular}}
\caption{
\revision{\editResultsTableCaption}
\revision{\editReminderShared}
...
SIRPF2 & 20 & \Sexpr{mysignif_ll_AIC(benchmark_model_summary$Model_no_juv[1],5)} & \Sexpr{mysignif_ll_AIC(benchmark_model_summary$Model_no_juv[2],6)} \\
...
Differences in AIC values between models are intepretable only between alternative models for the same data, so we fit statistical benchmark models to calibrate these results.
Specifically, we fit negative binomial regression models with linear, quadratic, and cubic dependence on time.
...
We also consider several variants of the SIRJPF2 model described here, namely a model that excludes the juvenile state (SIRPF2, described in Section S7.3), and a version of the SIRJPF2 model where the Gaussian noise has been replaced with Gamma noise (SIRJPF2-Gamma, see Section S9).
...
Table~S3 in the supplement presents a comparison of model fit and complexity across various configurations of unit-specific parameters within the PanelPOMP framework for the SIRJPF2 model. These distinct configurations represent varying hypotheses explaining the differences across replicates.
...
In our selected model, the feeding rate of the invasive {\it Daphnia} species ($f^{\invasive}_{S}$) is not statistically significantly different from that of the native {\it Daphnia} species ($f^{\native}_{S}$).
Also, the invasive species has a lower rate of infection per consumed spore ($p^{\native} < p^{\invasive}$), consistent with Fig.~1C of \citet{searle16}.
The native parameters—juvenile growth $r^{\native}$, feeding rate $f^{\native}_{S}$ and infection probability $p^{\native}$ show full identifiability as shown in Fig.~S1, whereas the corresponding invasive parameters $r^{\invasive},\,f^{\invasive}_{S},\,p^{\invasive}$ are only bounded on one side.
This limited identifiability is attributable to
(i) the lower density of \textit{D. lumholtzi} which may increase stochastic effects;
(ii) model complexity, since the parameters become identifiable in host-only experiments with simplified dynamics (Fig.~S8);
(iii) the product of these parameters governs the juvenile growth and infection process, which allows individual invasive coefficients to vary so long as their products remain fixed.
To investigate this last explanation, Table~\ref{tab:estimates} shows relatively precise identifiability of the effective infection rate ($p^{\native}f^{\native}_{S}$, $p^{\invasive}f^{\invasive}_{S}$), and the effective juvenile birth rate ($r^{\native}f^{\native}_{S}$, $r^{\invasive}f^{\invasive}_{S}$).
At the same time, the identifiability of the measurement error parameter ($\tau^{\native}_{S},\,\tau^{\invasive}_{S},\,\tau^{\native}_{I},\,\tau^{\invasive}_{I}$) explains variability that would otherwise inflate weakly identified process-noise terms.

Our results indicate that stochasticity in the juvenile stage ($\sigma^{\native}_{J}$ and $\sigma^{\invasive}_{J}$), food ($\sigma_{F}$) and parasite ($\sigma_{P}$) dynamics are important model features, needed to adequately describe the data (see the corresponding confidence intervals in Table~\ref{tab:estimates} and the profile likelihood plots in Fig.~S1).
That suggests the juvenile stage, food and parasite dynamics are particularly sensitive to unmodeled phenomena, such as lower fitness resulting from resource depletion, since unmodeled phenomena can be explained by the model as stochastic uncertainty.
Noise in other stages is small, and often insignificantly different from zero, as shown by profile likelihood plots for $\sigma^{\native}_{I}$, $\sigma^{\invasive}_{I}$.
Since the profile for $\sigma^{\native}_{S}$, $\sigma^{\invasive}_{S}$ is flat and insignificantly different from zero, we fixed the $\sigma^{\native}_{S}$ and $\sigma^{\invasive}_{S}$ to be zero as constants.
...
The log-likelihood estimates and corresponding AIC values for these models are presented in Supplementary Table~S3.
\bye{Under AIC and the chi-square likelihood-ratio test, making $\theta_I$ unit-specific appears favoured: it attains the lowest AIC in Table~S3, and the chi-square test gives $p=0.001$. However, these signals are unreliable for these high-dimensional comparisons fitted by Monte-Carlo particle filtering on a short eight-unit panel, where Wilks' regularity conditions fail. A calibrated parametric bootstrap, which provides the appropriate reference distribution here, finds that no unit-specific elaboration yields a statistically significant improvement over the all-shared model.}
Similar results hold for the other treatments (Tables~S6, S7, S9, S12, S13).
...
And the confidence intervals are obtained by the MCAP are shown in Fig.~S1 for the SIRJPF2 model, and Figs.~S7, S8, S15, S21, S22 for other treatments.
...
$\xi > 1$ could also be a result of reverse causation due to unmodeled heterogeneity: \textit{Daphnia} with higher filtering rate are exposed to more pathogen spores and so filtering rate can causally affect infection status in addition to any causal effect of infection status on filtering rate.
...
Our best-fitting model includes explicit modeling of juveniles and their maturation, a feature not present in the model of \citet{searle16}.
...
To make our analysis comparable to \citet{searle16}, we fitted to the same data, and therefore did not include a measurement model for juveniles.
...
Likelihood residual analysis is an effective tool for comparing alternative models by assessing differences in the conditional log-likelihood for each data point \citep{wheeler24,li24}.
We maximized the log-likelihood subject to the fixed values of $\xi_{J,{\unit}}$, and we compared the conditional log-likelihood for each observation category at each time point for each fixed $\xi_{J,{\unit}}$ value, as shown in Fig.~S3(a).
...
Moreover, plotting the conditional log-likelihood for each state at each time, we observe that all the log-likelihood components for susceptible and infected \textit{D. dentifera} and \textit{D. lumholtzi} are similar between the two parameter settings.
...
When a weakly identified parameter has consequences for the biological interpretation of the model, it is appropriate to fix that parameter at a biologically plausible value, and we therefore fixed $\xi_{J,{\unit}} = 1$ for subsequent analysis, reasoning that larger juveniles should have filtering rates of the same order of magnitude as adults.

\begin{figure}[t]
\centering
<<SimulationPlot,echo=FALSE, fig.width=7,fig.height=3,out.width="\\textwidth", cache=cache_option>>=
...
p1 = ggplot() +
  ...
  theme(axis.title = element_text(size = theme_size), axis.text.x = element_blank())+
  scale_x_continuous(limits = c(0, 52), breaks = c(0, 25, 50))
...
p3 = ggplot() +
  ...
  theme(axis.title = element_text(size = theme_size))+
  scale_x_continuous(limits = c(0, 52), breaks = c(0, 25, 50))
...
Shaded blue bands are $95\%$ pointwise confidence intervals; thick black lines show the mean; thin solid color lines are the experimental data.
...
If the previous shortage of such data analyses is evidence for the previous lack of effective methods, PanelPOMP methodology should facilitate many previously intractable scientific investigations.
...
Although we did not calibrate the model using juvenile data {\revision\editJuveniles}, the addition of latent $J^{\species}_{\unit}$ states leads to an improved statistical fit.
The inclusion of the juvenile stage provides a delay in the {\em Daphnia} population growth dynamics, and introduces competition between juveniles and adults for available food resources.
Juveniles rarely exhibit disease, perhaps because of the latency in the parasite's development within its host, but they nevertheless play a significant role in the infection dynamics.
...
Subsequent experimentation could substantiate or refute this prediction, and our data analysis methodology can assist such investigations.

Deterministic dynamic models continue to be widely used for ecological theory, despite the fact that inclusion of stochasticity can both improve model fit and enable diagnosis of model shortcomings \citep{king15}.
We demonstrate that including stochasticity can also lead to more parsimonious understanding of the system, since, in our case study, the dynamic stochasticity obviates a need for mesocosm-specific parameters to explain the difference between the experimental replicates.

\bye{The stochastic formulation does more than improve numerical fit; it reframes the ecological conclusions we draw from these data and makes them falsifiable. The deterministic model of \citet{searle16} attributes the rise and collapse of the \textit{Daphnia} populations to a density-dependent switch in reproductive strategy, from the asexual production of juveniles to the sexual production of dormant eggs. Our analysis instead favors a model in which the load-bearing mechanism is the depletion of the algal food resource coupled to age-structured recruitment: as the population grows it draws down the latent food compartment $F$, throttling juvenile recruitment and precipitating the crash, after which $F$ rebounds through resupply. Crucially, embedding this mechanism in a stochastic dynamic model turns the comparison between these rival explanations into a likelihood-based model selection rather than a narrative choice. Holding the food compartment fixed at its initial value (the no-$F$ ablation of Section~S5) removes the resource feedback while returning a free parameter to the model, yet it costs approximately $7.2$ log-likelihood units and is decisively worse by $\Delta\mathrm{AIC}\approx 12.4$. The model with a depletable food compartment is thus the decisively better-supported description of the observed peak-then-collapse trajectory. We emphasize, consistent with the caveat above, that this is a statement about statistical support and not about direct measurement: no food-level data were collected, so the depletion feedback remains an inference from the dynamics of the observed stages rather than a directly validated quantity, and subsequent experiments measuring food could substantiate or refute it.

Stochasticity is also what makes these mechanistic comparisons testable in the first place. Process noise in the juvenile, food and parasite dynamics absorbs the overdispersion and replicate-to-replicate variability that a deterministic model can only accommodate by positing systematic differences between otherwise identical replicates. Because the stochastic model treats each mesocosm as a conditionally independent realization of a shared dynamic process, likelihood-based model selection (Section~\ref{sec:res} and the supplement) lets us ask which mechanisms genuinely need to differ across replicates. For this eight-unit panel the answer is that none do: no unit-specific parameterization yields a statistically significant improvement over the all-shared model by \bye{a calibrated parametric bootstrap likelihood-ratio test (the asymptotic $\chi^2$ approximation is unreliable for these high-dimensional comparisons, as detailed in the supplement)}, so the heterogeneity between replicates is adequately explained by dynamic noise rather than by structural differences. Such hypothesis tests on the sources of variation are unavailable to a deterministic fit, which lacks a probability model for the data and therefore cannot separate process-level stochasticity from genuine between-unit structure. Embedding the mechanism in a stochastic dynamic model is therefore not a statistical refinement applied after the science is settled; it is the tool that lets us weigh resource depletion against a reproductive-strategy switch as competing, testable explanations of the same data.}

\bye{Methodologically, this analysis was made feasible by the marginalized panel iterated filter, whose block resampling of each unit's parameters respects the conditional independence of the panel units; we found this variant well-suited to likelihood maximization for our high-dimensional, weakly identified model, and a theoretical treatment is given by \citet{wheeler25}.}

This study has focused on the task of fitting models to understand the biological dynamics.
This is closely related to the task of forecasting: fitting by maximum likelihood is equivalent to assessing one-step forecasting skill, and the AIC criterion is designed to evaluate out-of-sample predictive skill \citep{aic74}.
Within this framework, the goals of model-based biological understanding and forecasting are therefore closely aligned.
...
\bye{Mechanistic models help scientists learn how ecological systems work, but fitting them to data is hard when key quantities go unobserved, dynamics are noisy and nonlinear, and several replicate systems must be analyzed together. We show that a recently developed simulation-based method, panel iterated filtering, makes rigorous likelihood-based analysis of such replicated experiments practical, and we apply a refinement that improves its reliability. Studying a freshwater experiment with two competing water-flea species, their algal food, and a shared fungal parasite, we find that accounting for randomness reshapes the ecological conclusions: depletion of the food resource, rather than a switch in reproductive strategy, best explains the observed boom and collapse, with chance variation alone explaining differences between replicates.}

\begin{acks}[Acknowledgments]
%% The corresponding author is Bo Yang with address 665 W 130th St, New York, NY 10027.
Aaron A. King is additionally affiliated with the Center for the Study of Complex Systems, University of Michigan.
This work was supported by National Science Foundation grant DMS-1761603.
\end{acks}


\begin{supplement}
\stitle{Data Availability Statement}
\sdescription{Code and data for the analysis is available on Github at https://github.com/Megumi-ybb/Daphnia-ms and is archived on Zenodo at https://doi.org/10.5281/zenodo.15595669.}
\end{supplement}
\begin{supplement}
\stitle{Supplementary Material}
\sdescription{The Supplementary Material includes detailed specification of the Panel Iterated Filtering algorithm and the benchmark models.
It also provides profile likelihoods, residual analyses, and diagnostics for the SIRJPF2, SIRJPF, and SRJF2 models, together with models for the additional dynamics discussed in the manuscript.}
\end{supplement}
