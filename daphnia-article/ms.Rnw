...
Partially observed Markov process (POMP) models, also known as hidden Markov models or state space models \citep{breto09,auger-methe21}, comprise a class of models suited to the description of such systems.
These models describe the latent process using a Markovian representation, a reasonable approach in many situations.
  A POMP model is mechanistic if it provides a causal description of the system, i.e., if it can explain the consequence of interventions to the system.
  ...
  For either of these goals, investigation of panel data leads to the challenging statistical task of fitting a collection of related nonlinear non-Gaussian vector-valued POMP models, called a PanelPOMP model \citep{breto20}.
...
Iterated filtering techniques apply to a more general class of POMP models, but the high-dimensional nature of PanelPOMP models adds additional methodological challenges.  
  ...
  We calculate Akaike's Information Criterion to compare the overall fit of rival mechanistic models.
...
This ability was unlocked by panel iterated filtering \citep{breto20} and further strengthened by the marginalized panel iterated filtering algorithm that we develop and demonstrate here.
These algorithms are discussed in more detail in Section~\ref{sec:meth}.
First, we discuss the experimental system in Section~\ref{sec:studysystem}.
Then, we present our models and their fitted parameter values in Section~\ref{sec:mecmod}.
...
The experimental units are replicates of the treatments; the design consisted of 10 replicates for single-host treatments and 9 replicates for two-host treatments.
...
Data from the four species treatment are displayed in Figure~\ref{fig:data_vis}.
...
However, resource depletion affects parthenogenetic reproduction as well influencing a switch to sexual reproduction, and we investigate the hypothesis that explicit modeling of resource depletion can help to explain the observed dynamics.
This led us to introduce a latent food variable, and we found that including age structure via a juvenile stage also helps to improve the statistical fit of the model.
...
Our hypothesized model is supported so far as it is the best available model according to this criterion; our model and data are available at \eic{IT'S TIME TO PUT A CLEAN VERSION OF THE REPO IN PUBLIC; LET'S DISCUSS THIS} to facilitate further investigations.
...
Figure~\ref{fig:flow_SIRJPF2} shows a flow diagram for a two-species model where the \textit{Daphnia} are susceptible adult (S), infected adult (I), or juvenile (J), with a superscript $\invasive$ for invasive and $\native$ for native.
...
The subscript $u$ ranges in $\seq{1}{U}$ with $U=8,9,10$ depending on the specific treatment.
...
We will later establish that this dependence is not needed; random dynamic noise is sufficient to explain the observed differences.
...
For the process model, this overdispersion is implemented by the noise parameters, $\sigma^{\species}_{I,{\unit}}$,  $\sigma^{\species}_{J,{\unit}}$, $\sigma^{}_{F,\unit}$, $\sigma^{}_{P,{\unit}}$.
...
We do not include the data on juveniles during the model fitting process, following the approach of \citet{searle16}.
...
Latent state estimation and parameter inference in POMP models is commonly carried out using the particle filter, also known as sequential Monte Carlo \citep{arulampalam02}.
...
Plug-and-play inference methods including iterated filtering, particle Markov chain Monte Carlo, and approximate Bayesian computing (ABC) are available using software such as pomp \citep{king16} or NIMBLE \citep{de17}.
...
For our example, $N_{\unit}=N=10$, and $t_{\unit,n}=t_n=5n+2$ days, though the algorithms and software do not need regular observation times.
...
For a model such as ours, the transition density is defined implicitly by the model equations (\ref{eq:model:first}--\ref{eq:model:last}).
The plug-and-play property means that we never require explicit evaluation of this density, only the ability to obtain simulated trajectories satisfying {eq:model:first}.
...
\caption{A flow diagram for panel iterated filtering. If the blue shaded blocks are omitted, this becomes a standard particle filter for each unit. Full pseudocode is in Section~S2. In the PIF algorithm of \citet{breto20}, the parameter resampling step involves re-selecting the unit-specific parameters corresponding to particle $j$ for units $\tilde u \neq u$ as well as $\psi_u$.
...
Here, we use Akaike's Information Criterion (AIC), defined to be twice the number of estimated parameters minus twice the maximized log-likelihood.
...
load('data/all_shared_parameter.rda')
...
parms = mif.estimate
AIC_target = -2 * loglik_target + 2 * length(parms)
...
$\sigma^{\native}_{S}$ & Standard deviation of Brownian motion of susceptible native adults &   $\sqrt{\mathrm{individual} \cdot \mathrm{day}^{-1}}$  & \Sexpr{mysignif(sigSn,2)}  & (\Sexpr{0},\Sexpr{mysignif(ci_table$sigSn[2],2)})\\
$\sigma^{\invasive}_{S}$ & Standard deviation of Brownian motion of susceptible invasive adult    &   $\sqrt{\mathrm{individual} \cdot \mathrm{day}^{-1}}$  & \Sexpr{mysignif(sigSi,2)}  & (\Sexpr{0},\Sexpr{mysignif(ci_table$sigSi[2],2)})\\
...
The negative binomial models with polynomial (quadratic or cubic) terms fit the dynamics better than the simpler linear version.
...
Table~\ref{Table:Model_comparison} includes the log-likelihood and AIC values for the deterministic ODE model of \citet{searle16}, which served as a starting point for our model.
Although the mechanistic model of \citet{searle16} was successfully used to provide scientific insight into the ecological dynamics of the experiment, we see that it does not provide a competitive statistical fit to the data compared to simple alternatives.
At the same time, we fit susceptible-infected-recovery-parasite-food (SIRPF2) model, which exclude the juvenile state from our SIRPF2 model as described in Supplementary S7.3.
The SIRPF2 model ignores the process of maturation of juvenile and only underscores the infection process of \textit{Daphnia} adult.
Even this model shows better statistical fit than the deterministic ODE model of \citet{searle16} and Negative Binomial model, it has weaker performance than SIPPF2 model.
A model that has a better statistical fit is expected to make relatively accurate predictions.
Every increase in model fit raises the bar for subsequent models, if they wish to use improved model fit to support new scientific insights.
At the same time, we fit susceptible-infected-recovery-parasite-food (SIRPF2) model, which exclude the juvenile state from our SIRPF2 model as described in Supplementary S7.3, which shows stronger statistical fit than other benchmark models expect SIRJPF2 model.
...
In our selected model, the native {\it Daphnia} species was found to have a higher feeding rate ($f^{\native}_{S}> f^{\invasive}_{S}$) and a lower rate of infection per consumed spore ($p^{\native} < p^{\invasive}$), consistent with Figure~1C of \citet{searle16}.
Our results indicate that stochasticity in the juvenile stage ($\sigma^{\native}_{J}$ and $\sigma^{\invasive}_{J}$) is an important model feature, needed to adequately describe the data (the corresponding confidence intervals in Table~\ref{tab:estimates} exclude zero; see also the profiles in Fig.~S1).
...
All the models we have presented here have included explicit modeling of juveniles and their maturation, although the model of \citet{searle16} did not do this. 
We found that the extra delay in the adult dynamics introduced by the juvenile stage was necessary for the model system to provide a strong statistical fit to the data.
\citet{searle16} collected juvenile data but did not include it in their data analysis. 
We make our analysis comparable to \citet{searle16} by fitting to the same data, and therefore we do not include a measurement model for juveniles.
This means that the juvenile data are available to us a check of model specification, and we show this in simulations from the fitted model in Fig.~\ref{fig:sim_plot}. 
...
To explore this assumption, we compared the value $\xi_{J,{\unit}} = 1$ (juveniles and adults filter at the same rate) with the extreme value $\xi_{J,{\unit}} = 0.001$ (juveniles filter much slower than adults).
We then maximized the log-likelihood over the remaining parameters subject to this constraint, and we compared the conditional maximized log-likelihood at each time for each observation category for the different $\xi_{J,{\unit}}$ values, as shown in Fig.~S3(a).
...
Thus, the similar log-likelihoods are not a consequence of a tradeoff between fitting different features of the data, but simply provide indistinguishable explanations of the fitted data.
However, the choice of $\xi_{J,{\unit}}$ nevertheless affects the interpretation of the model: the maximum likelihood estimates constrained to different values of $\xi_{J,{\unit}}$ lead to distinct behavior of juvenile \textit{Daphnia} in simulations, as shown in Fig.~S3(b).
...
When a weakly identified parameter has consequences for the biological interpretation of the model, it is appropriate to fix that parameter at a biologically plausible value, while bearing in mind that decision corresponds to an extra modeling assumption about which the data being fitted are agnostic.
...
Simulations of \textit{Daphnia} densities (Individuals$/$Liter), alga density ($10^6\cdot$cells$/$Liter) and parasite density ($10^3\cdot$spores$/$Liter), over time (days), in the mixed species parasitized dynamics based on estimated parameters, with $95\%$ pointwise confidence band (blue area) and simulated mean curves (solid black curves). 
Observed densities are represented by the solid colored curves.
The parasite and food densities were not observed, and the dashed colored curves represent a sample of simulated trajectories.
Individual simulation plots are shown in Fig.~S2.
}
\end{figure}

Based on AIC comparisons, we identified the optimal parameter swarm that reaches the best balance between fit and parsimony among unit-specific models and benchmark models. 
In addition to comparing model fit using information criterion, we further validate the calibrated SIRJPF2 model via a simulation study.
We generate 1000 simulations from the model and compare these simulations to the data (Figure \ref{fig:sim_plot}).

The simulation bands presented in Figure \ref{fig:sim_plot} closely align with the empirical data curves from the actual experiment, underscoring the model's capability to replicate the observed dynamics.
Notably, the model captures not only the trends but also the variability of the experimental data.
Supplementary Fig.~S2 further illustrate the fluctuation behavior of each states within the dynamics.
Simulations from other models considered are included in the supplementary Figure~S9--S12, S16, S17, S23--S26.


With added $J^{\species}_{\unit}$ states based on SIRPF2 model, our model provides an improved statistical fit and high coverage in observed juvenile population dynamics, even without inclusion of juvenile data.
As juveniles show rare infection by the parasite and gradually mature, this imposes limits on the growth speed of susceptible \textit{Daphnia} and further constrains the density growth rate of infecteds.
Further, juveniles compete with adults for available food resources.
While we have shown the relevance of age-structured dynamics for understanding the data on adults, future work could identify the juvenile dynamics more precisely (for example, improving identifiability of $\xi_{J,\unit}$) by fitting directly to juvenile data.
