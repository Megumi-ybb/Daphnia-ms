...
The simpler treatments are presented in Supplementary Sections~S8,~S9 and~S10.
...
The males arise predominantly later in the experiment as shown in Table~S-14.
...
Male \textit{Daphnia} are excluded from this model because they are absent until late in the experiment, are physically smaller than females, and have low observed infection rates.
...
A version of SIRJPF2 with gamma noise, called SIRJPF2-Gamma, is presented in Section~S13, though we found that a simpler Gaussian noise model is sufficient.
...
These experiment treatments and corresponding models are presented in Sections~S8, S9 and S10.
...
For the marginalized PIF (MPIF) algorithm, only $\phi$ and $\psi_u$ are reselected in this step.
}
\label{fig:pif}
\end{figure}
...
\end{tabular}}
\caption{
\byc{Summary of latent dynamic random variables and parameters in the SIRJPF2 model.
...
We also consider several variants of the SIRJPF2 model described here, namely a model that excludes the juvenile state (SIRPF2, described in Section~S11.3), and a version of the SIRJPF2 model where the Gaussian noise has been replaced with Gamma noise (SIRJPF2-Gamma, see Section~S13).
...
There are some substantial differences in the observed trajectories between experimental replicates (Figs.~\ref{fig:data_vis}, S-7, S-8, S-16, S-21, S-22).
...
Table~S-3 in the supplement presents a comparison of model fit and complexity across various configurations of unit-specific parameters within the PanelPOMP framework for the SIRJPF2 model. These distinct configurations represent varying hypotheses explaining the differences across replicates.
...
The native parameters—juvenile growth $r^{\native}$, feeding rate $f^{\native}_{S}$ and infection probability $p^{\native}$ show full identifiability as shown in Fig.~S-3, whereas the corresponding invasive parameters $r^{\invasive},\,f^{\invasive}_{S},\,p^{\invasive}$ are only bounded on one side.
...
(ii) model complexity, since the parameters become identifiable in host-only experiments with simplified dynamics (Fig.~S-10);
...
Our results indicate that stochasticity in the juvenile stage ($\sigma^{\native}_{J}$ and $\sigma^{\invasive}_{J}$), food ($\sigma_{F}$) and parasite ($\sigma_{P}$) dynamics are important model features, needed to adequately describe the data (see the corresponding confidence intervals in Table~\ref{tab:estimates} and the profile likelihood plots in Fig.~S-3).
...
The noise term is helpful when describing other dynamics, for example, dynamics with two species of \textit{Daphnia} and no parasite as shown in Supplement~S9.
...
By contrast, these parameters are better identified in the treatments without the parasite (Fig.~S-17).

Alternative  SIRJPF2 model specifications were fitted with various parameters taken to be unit-specific.
The log-likelihood estimates and corresponding AIC values for these models are presented in Supplementary Table~S-3.
\bye{Under AIC and the chi-square likelihood-ratio test, making $\theta_I$ unit-specific appears favoured: it attains the lowest AIC in Table~S-3, and the chi-square test gives $p=0.001$. However, these signals are unreliable for these high-dimensional comparisons fitted by Monte-Carlo particle filtering on a short eight-unit panel, where Wilks' regularity conditions fail. A calibrated parametric bootstrap, which provides the appropriate reference distribution here, finds that no unit-specific elaboration yields a statistically significant improvement over the all-shared model.}
\bye{Similar results hold for the other treatments (Tables~S-6, S-7, S-9, S-12, S-13), with a single exception: across all six model families and thirty unit-specific elaborations, the only statistically supported between-unit heterogeneity is unit-specific $\theta^{\native}_{I}$ in the \textit{D.~dentifera}-only system (Table~S-6; bootstrap $p=0.01$), which we note as a biologically meaningful feature of that system rather than carry into the shared baseline.}
...
And the confidence intervals are obtained by the MCAP are shown in Fig.~S-3 for the SIRJPF2 model, and Figs.~S-9, S-10, S-17, S-23, S-24 for other treatments.
...
We maximized the log-likelihood subject to the fixed values of $\xi_{J,{\unit}}$, and we compared the conditional log-likelihood for each observation category at each time point for each fixed $\xi_{J,{\unit}}$ value, as shown in Fig.~S-5(a).
...
The choice of $\xi_{J,{\unit}}$ nevertheless affects the interpretation of the fitted model: the maximum likelihood estimates constrained to different values of $\xi_{J,{\unit}}$ lead to distinct behavior of juvenile \textit{Daphnia} in simulations, as shown in Fig.~S-5(b).
If $\xi_{J,{\unit}}$ is set to be lower than is biologically plausible, simulated trajectories from the fitted model have far fewer juveniles than was observed, which is shown in Fig.~S-5(b) as well.
...
Unit level simulation plots are shown in Fig.~S-4.
...
Simulations from other models considered are included in the supplementary Figs.~S-11--S-14, S-18, S-19, S-25--S-28.
