...
\author{Bo Yang \authoraddress Department of Biostatistics, University of Michigan \\
        Jesse Wheeler  \authoraddress Department of Statistics, University of Michigan \&  \authoraddress Department of Mathematics and Statistic, Idaho State University\\
...
This study applies iterated particle filtering techniques to explore possibilities for likelihood-based statistical analysis of these complex systems.
We analyze time series data from six mesocosm experiments that includes two species of {\it Daphnia}, an alga and a fungal parasite.
Iterated filtering enables maximization of the likelihood for scientifically motivated nonlinear partially observed Markov process (POMP) models, providing access to standard likelihood-based methods for parameter estimation, confidence intervals, model selection and diagnostics.
This toolbox allows scientists to propose and evaluate scientifically motivated stochastic dynamic models for panel data, constrained only by specifying a measurement distribution describing how the system state is observed.
Existing Bayesian tools cannot handle the nonlinear POMP models, including ecological aspects. 
Our framework provides the a statistically efficient, likelihood-based solution, avoiding the information loss inherent in GMM or ABC approaches.
\end{abstract}
...
The Supplementary Material includes detailed Panel Iterated Filtering algorithm and the benchmark model specifications.
...
Code and data for the analysis is available at the Github repository: https://github.com/Megumi-ybb/Daphnia-ms or https://doi.org/10.5281/zenodo.15595669.
