...
Existing Bayesian tools cannot handle the nonlinear, partially observed Markov-process models, including ecological aspects. 
Our iterated-filtering framework provides the a statistically efficient, likelihood-based solution, avoiding the information loss inherent in GMM or ABC approaches.
\end{abstract}
...
At the same time, Bayesian method cannot accommodate the nonlinear, non-Gaussian mechanistic models considered here \citep{wigren22}, and no ecological study has yet achieved full-likelihood inference for them. 
