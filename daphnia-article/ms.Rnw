...
\revision{\editBayes}
  ...
  In doing so, we ensure that the proposed dynamic model provides a plausible mechanistic description of the dynamic system, supported by evidence that the model also provides a suitable quantitative description of data that arise from the system.
...
\begin{figure}[h!]
\centering
\bye{Figure~3 (TikZ block below) redesigned per Ed's suggestion (response.tex L279). The ``Resample states / parameters'' box now sits below the perturb-predict-weight stack. The Output box sits directly below the Resample box rather than to its left, because the leftward placement Ed suggested collided with the loop-back riser arrow at the \texttt{n=1:N} column. Loop-back arrows now exit \texttt{resampleProcess.east}, route around Output via a horizontal bus below the figure, and rise to \texttt{m=1:M}, \texttt{u=1:U}, \texttt{n=1:N}. The diagram renders at native size (no \texttt{\textbackslash resizebox}) with \texttt{font=\textbackslash small}, so the fonts are legible at the AOAS column width, which was Ed's underlying purpose.}
\begin{tikzpicture}[
    node distance=4cm,
    font=\small
  ]
...
% Perturb / predict / weight stack to the right of N
...
\node (perturb) [process2, above of=predict, yshift=-2.4cm] {Perturb parameters $\mathtt{\phi}$, $\mathtt{\psi_u}$};
\node (weight)  [process,  below of=predict, yshift=2.4cm]  {Calculate weights for data,\\ unit $\mathtt{u}$, time $\mathtt{t_{n}}$};
...
% Resample states / parameters: now BELOW the ppw stack rather than to its right
\node (resampleState)  [process,  below of=weight,        yshift=2.0cm] {Resample states};
\node (resampleParams) [process2, below of=resampleState, yshift=2.4cm] {Resample parameters};
...
% Output: directly below the resample box (terminal element of the pipeline).
% Placed in resampleProcess's column to avoid overlap with the loop-back risers
% at the M / U / N columns to its left.
\node (O) [mytitle, below of=resampleProcess, yshift=1.0cm]
    {\textbf{Output:} Monte Carlo Maximum\\ Likelihood Parameter Estimate};
\draw[arrow,black] (resampleProcess.south) -- (O.north);

% Loop-back arrows from resampleProcess back to N, U, M.
% Routed out of resampleProcess.east, around the right side of Output, then
% down to a horizontal bus below Output, then leftward to each target column,
% then up to the target. This keeps every loop-back segment clear of Output.
\coordinate (rightLane) at ([xshift=1.5cm] resampleProcess.east);
\coordinate (loopbus)   at ([yshift=-1.8cm] O.south);
\coordinate (busRight)  at (rightLane |- loopbus);
\draw[arrow,black] (resampleProcess.east) -- (rightLane) -- (busRight)
