...
In particular, likelihood residual analysis is especially effective in identifying model misspecification or weak parameter identifiability \citep{wheeler24,li24}.
We maximized the log-likelihood over the remaining parameters subject to this constraint to conduct the residual analysis.
The conditional maximized log-likelihood at each time for each observation category were compared for the different $\xi_{J,{\unit}}$ values, as shown in Fig.~S3(a).
