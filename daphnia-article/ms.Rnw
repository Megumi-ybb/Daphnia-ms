...
The simpler treatments are presented in Supplementary Sections~{\suppSecSIRJPF},~{\suppSecSRJFii} and~{\suppSecSRJF}.
...
The males arise predominantly later in the experiment as shown in Table~{\suppTablePeakDensityTiming}.
...
A version of SIRJPF2 with gamma noise, called SIRJPF2-Gamma, is presented in Section~{\suppSecSIRJPFgamma}, though we found that a simpler Gaussian noise model is sufficient.


The nonlinearity and intrinsic stochasticity of the SIRJPF2 model are characteristic features of dynamics in biological systems \citep{coulson04,wilkinson18}.
...
The experimentally determined sampling rate, $\delta = 0.013$ per day, and food replenishment rate, $\mu = 0.37 \times 10^6$ cells per liter per day, are fixed parameter values that are shared between units based on the experimental protocol.
{\footnotesize
\begin{align}
    ...
    - (\theta_{P,{\unit}} + \delta) P_{\unit}(t)\, dt + P_{\unit}(t)\, d\zeta_{P,{\unit}},\\
    dF_{\unit}(t) &= - \hspace{-1mm} \sum_{\species \in \{\native, \invasive\}} f^{\species}_{S,{\unit}} \, F_{\unit}(t) \left(S^{\species}_{\unit}(t)+\xi_{J,{\unit}} \, J^{\species}_{\unit}(t) +\xi_{\unit} \, I^{\species}_{\unit}(t)\right)dt  + \mu \, dt - \delta \, F_{\unit}(t) \, dt + F_{\unit}(t)\, d\zeta_{F,{\unit}},\\
...
Simpler treatments with a single \textit{Daphnia} species (SIRJPF) and without the parasite (SRJF and SRJF2) arise as special cases of SIRJPF2, with appropriate states and parameters.
These experimental treatments and corresponding models are presented in Sections~S7, S8 and S9.
...
\revision{\editMainTextTutorials}
...
If the blue shaded blocks are omitted, this becomes a standard particle filter for each unit. Full pseudocode is in Section~S2; the panel iterated filter and its marginalized variant differ only in the parameter resampling step, described in the text.
}
\label{fig:pif}
\end{figure}
...
Here, we use AIC, defined to be twice the number of estimated parameters ($2p$) minus twice the maximized log-likelihood.
...
\revision{\editLANandAICforPOMP}
%%
%% REMOVED TO MEET LENGTH CONSTRAINT
%% AIC penalizes complexity only so far as it affects generalization error for prediction; parsimony has additional scientific benefits, but the scientific value of model simplicity cannot be determined by a purely statistical criterion.
%%
...
$\beta^{\native}$       & Spores produced per infected native individual     &  $10^3 \cdot \mathrm{spores} \cdot \mathrm{individual}^{-1}$  & \Sexpr{mysignif(beta_n,2)} & \\
$\beta^{\invasive}$       & Spores produced per infected invasive individual   &  $10^3 \cdot \mathrm{spores} \cdot \mathrm{individual}^{-1}$  & \Sexpr{mysignif(beta_i,2)} & \\
...
Model & Parameters & Max log-likelihood & AIC \\ [0.5ex]
 \hline\hline
SIRJPF2 & 24 & \Sexpr{mysignif_ll_AIC(loglik_target,5)} & \Sexpr{mysignif_ll_AIC(AIC_target,6)} \\ % k=24: sigSn=sigSi=0 fixed at boundary, excluded; matches AIC=2k-2ll (26 would give 1813.12)
...
SIRJP2 & 23 & -887.8 & 1821.6 \\
SIRJPF2-Gamma & 26 & \Sexpr{mysignif_ll_AIC(benchmark_model_summary$SIRJPF2_gamma[1],5)} & \Sexpr{mysignif_ll_AIC(benchmark_model_summary$SIRJPF2_gamma[2],6)} \\ % k=26, not 24: this model has no sigSn/sigSi to fix at the boundary; its noise is transition-level (sigSIn, sigIPn, sigSIi, sigIPi, sigSJn, sigSJi), all estimated. Matches stored AIC=2k-2ll. Was \Sexpr{length(parms)}, which read the SIRJPF2 MLE object and gave 26 only by coincidence.
...
Parameters is the number of estimated parameters; values held fixed at a boundary are excluded, which is why SIRJPF2 counts $24$ while its gamma-noise variant, having no such boundary-fixed noise intensities, counts $26$.
}
...
Differences in AIC values between models are interpretable only between alternative models for the same data, so we fit statistical benchmark models to calibrate these results.
...
We also consider several variants of the SIRJPF2 model described here, namely a model that excludes the juvenile state (SIRPF2, described in Section~{\suppSecJablation}),
a model omitting the dynamics of the algal food (SIRJP2, see Section~{\suppSecFablation}),
and a version of the SIRJPF2 model where the Gaussian noise has been replaced with Gamma noise (SIRJPF2-Gamma, see Section~{\suppSecSIRJPFgamma}).


The SIRJPF2 model quantitatively describes the observed data better than any of the alternatives, while simultaneously providing a plausible mechanistic description of the experimental system.
\revision{\editNewModelComparisonExplanation}
Having assessed the fit of our model, we have some support for interpreting the estimated parameters of the model.
We proceed to do this, acknowledging the caveat that it is hard to rule out the possibility that some biological details in the model are misspecified in ways that may lead to confounding with the modeled mechanisms and therefore bias in the causal interpretation of estimated parameters \citep{wheeler24}.

There are some substantial differences in the observed trajectories between experimental replicates (Figs.~\ref{fig:data_vis}, {\suppFigSIRJPFdentiferaData}, {\suppFigSIRJPFlumholtziData}, {\suppFigSRJFiiData}, {\suppFigSRJFdentiferaData}, {\suppFigSRJFlumholtziData}).
...
Table~{\suppTableUnitSpecificTest} in the supplement presents a comparison of model fit and complexity across various configurations of unit-specific parameters within the PanelPOMP framework for the SIRJPF2 model. These distinct configurations represent varying hypotheses explaining the differences across replicates.
...
Also, the invasive species has a higher rate of infection per consumed spore ($p^{\native} < p^{\invasive}$), consistent with Fig.~1C of \citet{searle16}.
The native parameters---juvenile growth $r^{\native}$, feeding rate $f^{\native}_{S}$ and infection probability $p^{\native}$---are fully identified, as shown in Fig.~{\suppFigSIRJPFiiProfiles}, whereas the corresponding invasive parameters $r^{\invasive},\,f^{\invasive}_{S},\,p^{\invasive}$ are only bounded on one side.
...
(ii) model complexity, since the parameters become identifiable in host-only experiments with simplified dynamics (Fig.~\suppFigSIRJPFlumholtziProfiles);
...
Our results indicate that stochasticity in the juvenile stage ($\sigma^{\native}_{J}$ and $\sigma^{\invasive}_{J}$), food ($\sigma_{F}$) and parasite ($\sigma_{P}$) dynamics are important model features, needed to adequately describe the data (see the corresponding confidence intervals in Table~\ref{tab:estimates} and the profile likelihood plots in Fig.~\suppFigSIRJPFiiProfiles).
...
The noise term is helpful when describing other dynamics, for example, dynamics with two species of \textit{Daphnia} and no parasite as shown in Section~\suppSecSRJFii.
...
By contrast, these parameters are better identified in the treatments without the parasite (Fig.~\suppFigSRJFiiProfiles).

Alternative  SIRJPF2 model specifications were fitted with various parameters taken to be unit-specific.
The log-likelihood estimates and corresponding AIC values for these models are presented in Supplementary Table~{\suppTableUnitSpecificTest}.
\revision{\editParametricBootstrap}
Similar results hold for the other treatments (Tables~{\suppTableSIRJPFdentiferaUnitSpecific}, {\suppTableSIRJPLumholtziUnitSpecific}, {\suppTableSRJFiiUnitSpecific}, {\suppTableSRJFdentiferaUnitSpecific}, {\suppTableSRJFlumholtziUnitSpecific}).
...
The confidence intervals are obtained by MCAP, as shown in Fig.~{\suppFigSIRJPFiiProfiles} for the SIRJPF2 model, and Figs.~{\suppFigSIRJPFdentiferaProfiles}, {\suppFigSIRJPFlumholtziProfiles}, {\suppFigSRJFiiProfiles}, {\suppFigSRJFdentiferaProfiles}, {\suppFigSRJFlumholtziProfiles} for other treatments.
...
A subsequent investigation extending the model and measurements to include \textit{Daphnia} size and genotype could help to clarify this.
...
The parameter is weakly identified: fixing $\xi_{J,{\unit}}=1$ or a much smaller value changes the maximized log-likelihood only slightly and leaves the conditional log-likelihoods for susceptible and infected \textit{Daphnia} similar (Figure~{\suppFigDiagnostics}), but the smaller value produces far fewer juveniles in simulation than were observed, which would bias the estimates of $r^{\species}_{\unit}$, $p^{\species}_{\unit}$ and $f^{\species}_{S,\unit}$.
...
\begin{figure}[t]
\centering
<<SimulationPlot,echo=FALSE, fig.width=7,fig.height=3,out.width="0.9\\textwidth", cache=cache_option>>=
...
Simulated \textit{Daphnia} densities (Individuals$/$Liter), alga density ($10^6\cdot$cells$/$Liter) and parasite density ($10^3\cdot$spores$/$Liter), over time (days), in the mixed species parasitized dynamics with parameters from Table~\ref{tab:estimates}.
...
Unit level simulation plots are shown in Fig.~\suppFigSimulations.
...
Simulations from other models considered are included in the supplement (Figs.~{\suppFigSIRJPFdentiferaSimAggregate}--{\suppFigSIRJPFlumholtziSimByUnit}, {\suppFigSRJFsimAggregate}, {\suppFigSRJFdentiferasimByUnit}, {\suppFigSRJFdentiferaSimAggregate}--{\suppFigSRJFlumholtziSimByUnit}).
...
This encourages us to explore the dynamics of latent states in the dynamic system by simulating the unobserved densities of \textit{M.~bicuspidata} and \textit{A.~falcatus} (see the fourth column of Fig.~\ref{fig:sim_plot}).
...
As mentioned in Section~\ref{sec:studysystem}, high population density of \textit{Daphnia} and limited food resources will prompt the production of male \textit{Daphnia} leading to sexual reproduction and the generation of diapausing eggs.
...
A. A. King is affiliated with the Center for the Study of Complex Systems, University of Michigan.
This work was supported by National Science Foundation grants DMS-1761603 and OAC-2607869.
\end{acks}

\section*{Data Availability Statement}
Code and data for the analysis is available on Github at \url{https://github.com/Megumi-ybb/Daphnia-ms} and is archived on Zenodo at \url{https://doi.org/10.5281/zenodo.21754618}.

\section*{Supplementary Material}
Supporting text gives details of the Panel Iterated Filtering algorithm and the benchmark models.
