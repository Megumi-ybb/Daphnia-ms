...
\bye{A central scientific finding from our analysis is that mesocosms behave as stochastic replicates of a single system: across all treatment-by-species combinations considered, a model with parameters shared across replicate mesocosms is preferred over models with mesocosm-specific parameters under both AIC and a parametric bootstrap likelihood ratio test. Empirical coverage of profile likelihood confidence intervals, assessed by simulation from the fitted model, supports the interpretation of the parameter estimates and confidence intervals reported for the four interacting species.}
\end{abstract}
...
\begin{figure}[ht]
\centering
\bye{Figure~1 (\texttt{<<DataVis>>} chunk below) redesigned per AE: now three rows -- adult susceptible, adult infected, juveniles. Uniform line-type / colour mapping (\textit{D.~dentifera} solid blue, \textit{D.~lumholtzi} dashed black) so the figure reads in grayscale. Review owed before submission.}
...
\bye{The experimental data.
Density (individuals per litre) of adult susceptible females (top), adult infected females (middle) and juveniles (bottom).
\textit{D.~dentifera} is plotted with solid lines (blue in the colour version), and \textit{D.~lumholtzi} with dashed lines (black in the colour version).
There were negligible infected juveniles and males.
Columns are mesocosms corresponding to replications under the same treatment setting.}
}
\end{figure}
...
\begin{figure}[h!]
\centering
\bye{Figure~3 (TikZ block below) redesigned per Ed's suggestion (response.tex L279): the ``Resample states / parameters'' box now sits below the perturb-predict-weight box, the Output box is to its left, and the diagram renders at native size (no \texttt{\textbackslash resizebox}) with \texttt{font=\textbackslash small}, so the fonts are legible at the AOAS column width. Review the rendered figure for arrow alignment before submission.}
\begin{tikzpicture}[
    node distance=4cm,
    font=\small
  ]
...
% Output: to the LEFT of the resample box
\node (O) [mytitle, left of=resampleProcess, xshift=-0.5cm]
    {\textbf{Output:} Monte Carlo Maximum\\ Likelihood Parameter Estimate};
\draw[arrow,black] (resampleProcess.west) -- (O.east);

% Loop arrows: from resampleProcess back to N, U, M
\node (resampleProcess_south1) [below of=resampleProcess, yshift= 1.5cm] {};
\node (resampleProcess_south2) [below of=resampleProcess, yshift= 1.0cm] {};
\node (resampleProcess_south3) [below of=resampleProcess, yshift= 0.5cm] {};
\node (N_south) [below of=N, yshift= 1.5cm] {};
\node (U_south) [below of=U, yshift= 1.0cm] {};
\node (M_south) [below of=M, yshift= 0.5cm] {};
