...
\jwc{I don't think I understand the next sentence. What are we referring to here when we say parameter swarm? Do we mean that we use AIC to pick model structure and the number of unit-specific parameters? Perhaps we can just remove the sentence.}
...
\jwc{Check for consistency between the use of Figure and Fig. in the manuscript.}
...
\jwc{Rework this paragraph.}
With added $J^{\species}_{\unit}$ states based on SIRPF2 model, our model provides an improved statistical fit and high coverage in observed juvenile population dynamics, even without directly calibrating the model using juvenile data.
