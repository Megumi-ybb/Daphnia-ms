...
The two diagonal entries are the quantities of interest: AIC's specificity under the all-shared truth, and its power under a genuinely unit-specific truth. Under the all-shared truth AIC recovers the correct all-shared model on $78\%$ of panels (mean $\Delta\mathrm{AIC}=+5.0$ in its favour), over-selecting the $\theta_I^{\native}$-unit-specific elaboration on the remaining $22\%$; under the unit-specific truth it recovers the unit-specific model on $92\%$ of panels (mean $\Delta\mathrm{AIC}=-12.9$). AIC therefore detects genuine unit-specific heterogeneity reliably, but spuriously prefers the more complex model on about one panel in five when none is present, which is precisely why a raw AIC or asymptotic $\chi^2$ advantage must be checked against the calibrated bootstrap likelihood-ratio test of Table~\ref{Table:SIRJPF2_model_comparison}, which on the observed data finds that no unit-specific elaboration improves on the all-shared model.
...
\section{\texorpdfstring{$F$}{F} Ablation}\label{sec:no-F-ablation}
...
Effectively, this eliminates from the dynamics food loss due to consumption by {\it Daphna}, food renewal inflow, and the process noise $\sigma_F$.
...
One caveat concerns $\theta_I$, the closest of the seven alternatives to significance: at $B=100$ its bootstrap $p$-value of $0.10$ carries its own Monte Carlo uncertainty, and we are increasing the number of bootstrap replicates to estimate it more precisely.
