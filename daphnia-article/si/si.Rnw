...
The two diagonal entries are the quantities of interest: AIC's specificity under the all-shared truth, and its power under a genuinely unit-specific truth. We will report them here once the simulation completes, and connect them to the bootstrap likelihood-ratio finding that, on the observed data, no unit-specific elaboration improves on the all-shared model.}
...
Against this reference neither $\theta_I$ ($p_{\mathrm{boot}}=0.13$) nor $p$ ($p_{\mathrm{boot}}=0.41$) is significant, and we retain the all-shared model.

The disagreement between the two references is visible in the bootstrap null itself. 
For the fourteen-parameter alternatives the simulated $\Lambda$ does not sit near the $\chi^2_{14}$ mean of $14$: its average across these alternatives is about $28$, ranging from roughly $17$ for $\theta_S$ to $39$ for $r$. 
...
Measured against its own bootstrap null, which is centred near $26$, the observed $\Lambda=35$ for $\theta_I$ is unremarkable, even though it is extreme against $\chi^2_{14}$.

That this is a failure of the asymptotics rather than an artifact of our search is confirmed by a low-dimensional, well-identified control. 
For $\theta_P$, which adds only seven parameters, the bootstrap null is close to $\chi^2_7$---its mean is about $7$---and the two references agree ($p=0.23$ against $p_{\mathrm{boot}}=0.28$). 
Where Wilks' conditions hold the bootstrap reproduces the chi-square, and where they fail it corrects it.
...
One caveat concerns $\theta_I$, the closest of the seven alternatives to significance: at $B=100$ its bootstrap $p$-value of $0.13$ carries its own Monte Carlo uncertainty, and we are increasing the number of bootstrap replicates to estimate it more precisely. 
