...
To investigate identifability, we carried out an additional exploration to obtain estimates, profiles and confidence intervals for the composite parameters,  $r^{\species}f^{\species}_{S}$ and $r^{\species}f^{\species}_{S}$, as well as the individual parameters.
