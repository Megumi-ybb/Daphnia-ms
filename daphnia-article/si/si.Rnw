...
\bye{We assess the finite-sample coverage of the Monte Carlo Adjusted Profile (MCAP) confidence intervals by parametric bootstrap: we simulate $B=100$ datasets from the fitted all-shared SIRJPF2 maximum likelihood estimate and, on each, recompute the MCAP profile and check whether its $95\%$ interval contains the data-generating value, for the native and invasive birth rates $r^{\native}$ and $r^{\invasive}$, the parasite spore process noise $\sigma_P$, and the food process noise $\sigma_F$.
A profile confidence interval is the set of parameter values whose profile log-likelihood lies within a \emph{cutoff} $c$ of the maximum, $\{\theta: \ell^{\max}-\ell(\theta) \le c\}$; the \emph{basic} MCAP interval uses the asymptotic cutoff $c = \chi^2_{1,0.95}/2 = 1.92$.
Where this under-covers, we also report a \emph{corrected} interval that replaces $1.92$ by the cutoff calibrated to attain $95\%$ coverage in the simulation---the $95$th percentile of the simulated profile drop at the truth, $D = \ell^{\max}-\ell(\theta_{\text{true}})$.
A larger cutoff gives a wider interval, so the corrected interval is the same profile cut at a higher threshold.
Table~\ref{Table:mcap-coverage} reports both.
For $r^{\invasive}$ the profile is flat---it enters the process model only through the product $r^{\invasive} f_S^{\invasive}$ (and $p^{\invasive}$ only through $p^{\invasive} f_S^{\invasive}$), so its level is not separately identifiable and its nominal $95.8\%$ coverage is a degenerate artifact; we omit it from the correction.}
...
$\theta_{P,\unit}$                     & $4.48$  & $0.72$  & $0.70$ \\
$\theta^{\species}_{I,\unit}$          & $25.6$  & $0.029$ & $0.05$ \\
$p^{\species}_{\unit}$                 & $25.0$  & $0.034$ & $0.06$ \\
$r^{\species}_{\unit}$                 & $-6.75$ & $1.00$  & $0.99$ \\
$f^{\species}_{S,\unit}$               & $-1.93$ & $1.00$  & $0.98$ \\
\bottomrule
\end{tabular}
\end{table}}
