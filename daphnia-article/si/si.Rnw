...
The 22-dimensional parameter is fit to an $80$-observation panel ($U=8$ units, $N_u = 10$ time points each), so the asymptotic Wilks expectation for $\ell_{\text{MLE}} - \ell_{\text{truth}}$ is $p/2 = 11$.
...
\bye{In addition to the four target parameters, we examined the within-chain dispersion of all 22 free parameters across the 20 iterated-filtering chains used in the diagnostic study.
...
\bye{Inline import of the four PDF coverage plots (\texttt{coverage\_plot\_\{ri,rn,sigP,sigF\}.pdf} from \texttt{Mixed-species/SIRJPF2/coverage\_study/coverage\_results/}) and the formatted three-log-likelihood diagnostic table is owed; raw data already exist on disk.}
