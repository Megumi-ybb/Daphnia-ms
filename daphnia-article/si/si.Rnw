...
For three of the four parameters the 95\% MCAP confidence interval covers the true value in the majority of simulations: 77.8\% for $r^{\native}$, 87.0\% for $\sigma_P$, and 74.0\% for $\sigma_F$.
For $r^{\invasive}$ the profile is essentially flat: in the SIRJPF2 process model $r^{\invasive}$, $f_S^{\invasive}$ and $p^{\invasive}$ enter the rate term $J^{\invasive}$ only through their product, so individual MCAP confidence intervals span the entire parameter grid and the nominal 95.8\% empirical coverage is degenerate rather than informative.
We therefore report coverage results only for the three identifiable parameters in the main response and explicitly document the $r^{\invasive} \times f_S^{\invasive} \times p^{\invasive}$ product ridge here.}
...
The observed gap of $\approx 8.5$ is therefore within the asymptotic envelope; it represents the standard finite-sample displacement of the maximum likelihood estimate from the true parameter on identifiability ridges, not a methodological failure of MCAP itself.}
...
Inspecting the SIRJPF2 process model, these three parameters appear only through the product
$r^{\invasive} f_S^{\invasive} p^{\invasive} F$
in the juvenile, infected and susceptible terms for the invasive species, so their levels are non-identifiable individually.
...
The remaining identifiable parameters ($r^{\native}$, $f_S^{\native}$, $p^{\native}$, $\sigma_P$, $\sigma_F$) had within-chain standard deviations of $1.4$--$1.5$ log units, with finite-sample log-MLE drift of order $\pm 1$.}
...
\end{tabular}
\caption{\label{fig:mcap-coverage-plots}Empirical coverage of MCAP 95\% confidence intervals across $B=100$ bootstrap datasets simulated from the SIRJPF2 all-shared MLE, for the four target parameters. In each panel, the horizontal lines mark each simulated dataset's MCAP confidence interval, ordered by midpoint; the vertical solid line marks the true parameter value; a CI ``covers'' if it intersects the true value. Panels~(a)--(c) show identifiable parameters with finite-sample undercoverage explained by Wilks-scale MLE displacement (see Table~\ref{Table:three-loglik}). Panel~(d) shows the $r^{\invasive}$ profile, which is flat on the parameter axis because $r^{\invasive}$ enters the SIRJPF2 process model only through the product $r^{\invasive} f_S^{\invasive} p^{\invasive}$.}
\end{figure}
