...
# Set global caching
# cache_option = FALSE
cache_option = FALSE # only use when editing text
...
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(MPIF)} & \makecell[c]{AIC\\(MPIF)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\varnothing$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[1,]$lrt_pval)} & --- \\
$\theta^{\species}_{I,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[2,]$lrt_pval)} & 0.13 \\
$p^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[3,]$lrt_pval)} & 0.41 \\
$\theta_{P,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[4,]$lrt_pval)} & 0.28 \\
$\xi_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[5,]$lrt_pval)} & 0.95 \\
$\theta^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[6,]$lrt_pval)} & 0.46 \\
$f^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[7,]$lrt_pval)} & 0.97 \\
$r^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[8,]$lrt_pval)} & 0.94 \\[1ex]
 \hline
\end{tabular}
\end{center}
...
\bye{Under the standard (unmarginalized) panel iterated filtering, the all-shared model ($\varnothing$) attains the lowest AIC among all configurations and no unit-specific parameterization improves significantly upon it. Under the marginalized panel iterated filtering (MPIF) reported in the (MPIF) columns---the method better suited to fitting unit-specific panel parameters, introduced earlier in this supplement---the comparison changes. Making $\theta_I$ unit-specific attains the lowest AIC ($1802.1$ versus $1809.1$ for the all-shared model), with a significant likelihood-ratio test ($\chi^2$ $p=0.001$), and making $p$ unit-specific is also significant ($\chi^2$ $p=0.009$); the remaining parameters ($\theta_P$, $\xi$, $\theta_S$, $f_S$ and $r$) show no significant improvement, so the all-shared specification is retained for them. This points to genuine between-unit heterogeneity in $\theta_I$ (and, more weakly, $p$) that the marginalized fitting detects but the unmarginalized PIF underfits and so misses---a concrete instance of the methodological gain from MPIF. The $\theta_I$ heterogeneity is supported by both AIC and the $\chi^2$ likelihood-ratio test; the corresponding block-resampled parametric-bootstrap $p$-values, which avoid the $\chi^2$ asymptotic approximation, are being finalized and provisionally place $\theta_I$ and $p$ near the $0.05$ threshold (Table~\ref{Table:SIRJPF2_boot_provisional}), so the strength of the $\theta_I$/$p$ evidence should be weighed in light of both.}
