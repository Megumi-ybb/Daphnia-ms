...
# Uses the marginalized (MPIF / block) log-likelihood and AIC. Block resampling is a
# no-op for the all-shared null (no unit-specific parameters), so block_ll/block_AIC are
# NA there; we fall back to the ordinary ll/AIC, which equal the marginalized values for
# that model. This keeps the LRT a like-for-like MPIF-vs-MPIF comparison.
...
For $r^{\invasive}$ the profile is flat---it enters the process model only through the product $r^{\invasive} f_S^{\invasive}$ (and $p^{\invasive}$ only through $p^{\invasive} f_S^{\invasive}$), so its level is not separately identifiable and its nominal $94.0\%$ coverage is a degenerate artifact; we omit it from the correction.}

\begin{table}[ht]
\centering
\small
\setlength{\tabcolsep}{4pt}
\caption{\label{Table:mcap-coverage}\bye{Parametric-bootstrap coverage of the MCAP $95\%$ confidence intervals (log scale) across $B=100$ datasets simulated from the SIRJPF2 all-shared maximum likelihood estimate. The \emph{basic} interval uses the asymptotic cutoff $c=1.92$ and reproduces Table~1 of the main text; the \emph{corrected} interval re-cuts the same profile at the calibrated cutoff $c$ that targets $95\%$ coverage. The native birth rate $r^{\native}$ is biased high and the noise parameters $\sigma_P,\sigma_F$ biased low, all on weakly-identified ridges; $r^{\invasive}$ is non-identifiable (flat profile) and omitted from the correction.}}
\begin{tabular}{lccccccc}
\toprule
 & Truth & Mean MCAP & \multicolumn{2}{c}{Basic ($c=1.92$)} & \multicolumn{3}{c}{Corrected} \\
\cmidrule(lr){4-5}\cmidrule(lr){6-8}
Parameter & (log) & MLE (log) & $95\%$ CI & cov. & cutoff $c$ & $95\%$ CI & cov. \\
\midrule
$r^{\native}$   & $3.71$  & $4.17$  & $[3.27,\,5.27]$   & $88.0\%$ & $2.79$ & $[3.12,\,5.71]$   & $95.0\%$ \\
$\sigma_P$      & $-1.30$ & $-1.72$ & $[-3.12,\,-0.87]$ & $87.0\%$ & $3.91$ & $[-3.52,\,-0.56]$ & $95.0\%$ \\
$\sigma_F$      & $-1.94$ & $-2.49$ & $[-2.92,\,-1.46]$ & $76.0\%$ & $4.78$ & $[-4.11,\,-1.06]$ & $95.0\%$ \\
$r^{\invasive}$ & $12.28$ & flat    & \multicolumn{5}{c}{non-identifiable; nominal $94.0\%$ coverage degenerate} \\
\bottomrule
\end{tabular}
\end{table}

\bye{Two points frame these numbers. First, the undercoverage is a genuine finite-sample displacement of the maximum likelihood estimate along weakly-identified ridges, not a numerical artifact: a three-log-likelihood diagnostic on representative simulated datasets shows the production profiles reach a higher likelihood than a 20-chain global search warm-started at the truth (so they are not under-converged), and the implied MLE-to-truth gap of about $4.5$--$8.5$ log-likelihood units sits below the asymptotic Wilks envelope $p/2 = 12$ for the $p = 24$ free parameters---consistent with a weakly-identified model on a short, eight-unit panel rather than a failure of the MCAP construction. Second, the corrected interval is a parametric-bootstrap calibration from the fitted estimate, reliable only insofar as the model is correctly specified and $\hat\theta$ approximates the truth, with a cutoff that is itself noisy at $B = 100$; recalibrating the cutoff on a random half of the datasets and measuring coverage on the held-out half gives about $93\%$ out-of-sample coverage (mean over $500$ splits; $93.0$--$93.3\%$ across the three parameters), so the correction captures a genuine, generalisable feature rather than overfitting its own simulations. We therefore read the parametric-bootstrap coverage as an honest, exploratory characterisation of uncertainty: with several weakly-identified parameters, the analysis maps which mechanisms the panel identifies and quantifies their finite-sample uncertainty rather than certifying $95\%$ coverage for downstream decisions. The practical content of Table~\ref{Table:mcap-coverage} is that attaining nominal coverage in this richly parameterised model requires intervals roughly $1.3$ to $2.1$ times wider than the asymptotic ones; we report the basic MCAP interval as the primary, directly comparable estimate and the corrected interval as a transparent measure of that finite-sample cost.}
...
\end{tabular}
\caption{\label{fig:mcap-coverage-plots}Empirical coverage of MCAP 95\% confidence intervals across $B=100$ bootstrap datasets simulated from the SIRJPF2 all-shared MLE, for the four target parameters. In each panel, the horizontal lines mark each simulated dataset's MCAP confidence interval, ordered by midpoint; the vertical solid line marks the true parameter value; a CI ``covers'' if it intersects the true value. Panels~(a)--(c) show identifiable parameters with finite-sample undercoverage driven by displacement of the profile maximum likelihood estimate along weakly-identified ridges---$r^{\native}$ biased high, $\sigma_P$ and $\sigma_F$ biased low, with the largest displacement and the lowest coverage for $\sigma_F$---which the calibrated cutoff widens the interval to correct. Panel~(d) shows the $r^{\invasive}$ profile, which is flat on the parameter axis because $r^{\invasive}$ enters the SIRJPF2 process model only through the recruitment product $r^{\invasive} f_S^{\invasive}$.}
\end{figure}}
...
\eic{ALSO PUT HERE THE EXPLANATION OF THE SIMULATION-BASED LRT?}

\section{A toy model suggesting why the LRT is not chi-square when testing for unit-specific parameters}
...
Under $H_1$, the likelihood is maximized when $\tau=0$ for all $U$ and $N$.
Therefore, the MLE is inconsistent in a limit as $U$ and/or $N$ increase to infinity.
...
\begin{figure}[h]
\centering
<<fig-lrt-sim, fig.width=7,fig.height=4,out.width="\\textwidth", echo=FALSE, cache=cache_option>>=
...
Histogram of the likelihood ratio statistic (twice the difference of the maximized log-likelihoods) for simulations from $H_0$ with $U=\Sexpr{U}$, $N=\Sexpr{N}$, $\sigma=\Sexpr{sigma}$, $\tau = \Sexpr{tau}$..
...
$\theta^{\species}_{I,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_AIC,6)} & 0.001 & 0.13 \\
$p^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_AIC,6)} & 0.009 & 0.41 \\
$\theta_{P,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_AIC,6)} & 0.23 & 0.28 \\
$\xi_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_AIC,6)} & 0.92 & 0.95 \\
$\theta^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_AIC,6)} & 0.26 & 0.46 \\
...
$r^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_AIC,6)} & 0.12 & 0.94 \\[1ex]
 \hline
\end{tabular}
\end{center}
...
\bye{Under the standard (unmarginalized) panel iterated filtering the all-shared model ($\varnothing$) attains the lowest AIC; under the marginalized panel iterated filtering (MPIF) reported in the (MPIF) columns, making $\theta_I$ unit-specific attains the lowest AIC ($1802.1$ versus $1809.1$ for the all-shared model) and the $\chi^2$ likelihood-ratio test flags $\theta_I$ ($p=0.001$) and $p$ ($p=0.009$), with no other parameter improving significantly. The $\chi^2$ and bootstrap references in Table~\ref{Table:SIRJPF2_model_comparison} disagree for $\theta_I$ and $p$, and the $\chi^2$ reference is not reliable in this setting: Wilks' theorem presumes an exactly maximized likelihood, interior and well-identified parameters, and a sample size large relative to the parameter dimension, none of which holds for a fourteen-parameter unit-specific extension fit by Monte Carlo particle filtering on an eight-unit panel. The block-resampled parametric bootstrap replaces the asymptotic reference with the empirical null distribution of $\Lambda$, obtained by refitting both models---with the identical estimation procedure and particle count---to $B=100$ datasets simulated from the fitted all-shared null; it is therefore the appropriate test, and it places $\theta_I$ ($p_{\mathrm{boot}}=0.13$) and $p$ ($0.41$) well above the $0.05$ level, supporting the all-shared specification. The disagreement is explained by the shape of this empirical null: for the fourteen-degree-of-freedom alternatives the simulated $\Lambda$ is centred near $26$ rather than the $\chi^2_{14}$ mean of $14$, because maximizing over many added unit-specific parameters by stochastic search absorbs Monte Carlo and finite-sample fluctuations that an exactly maximized likelihood would not. This excess grows with both the number of added parameters and their identifiability---it is largest for the weakly identified recruitment parameters $r$ and $f_S$ (simulated mean $\Lambda$ roughly $2.5$ times the degrees of freedom) and mildest for the better-identified $\theta_S$---so the asymptotic $\chi^2$ increasingly overstates significance, and the observed $\Lambda=35$ for $\theta_I$, extreme against $\chi^2_{14}$, is unremarkable against the true null. That this is a genuine failure of the asymptotics rather than a numerical artifact is confirmed by the lower-dimensional, well-identified control $\theta_P$ (seven degrees of freedom), for which the bootstrap null coincides with $\chi^2_7$ and the two references agree ($0.23$ versus $0.28$). We accordingly take the bootstrap as the primary reference and report the $\chi^2$ values as a familiar but, in this regime, anti-conservative benchmark.}
