...
% \bye is the revision-pass flag used in the response letter and main text.
% Renders in purple so it visually separates from the existing \byc note macro.
% Implemented as a colour group rather than \textcolor{...}{...} so the body
% can span multiple paragraphs without LaTeX's "paragraph ended before
% \@textcolor was complete" error.
...
# LRT helper: compute p-value comparing alternative (unit-specific) vs null (all-shared) model
...
# Add LRT p-values to a parameter table (null model identified by rowname "all_shared")
add_lrt_pvalues <- function(ptable) {
  null_row <- which(rownames(ptable) == "all_shared")
  null_ll <- ptable[null_row, "block_ll"]
  null_AIC <- ptable[null_row, "block_AIC"]
  ptable$lrt_pval <- sapply(1:nrow(ptable), function(i) {
    if (i == null_row) return(NA)
    lrt_pvalue(ptable[i, "block_ll"], ptable[i, "block_AIC"], null_ll, null_AIC)
  })
  ptable
}
...
# Attach simulation-based (parametric bootstrap) LRT p-values to a parameter table.
# Reads bootstrap_pvalues.rds (produced by each family's collect_lrt.R) and joins
# the p_boot column to ptable by row index using alt_to_row mapping.
# If the file doesn't exist (HPC jobs not yet complete) the column is all NA,
# which format_pvalue() renders as "---".
...
\bye{We assess the empirical coverage of Monte Carlo Adjusted Profile (MCAP) confidence intervals on the SIRJPF2 model using a parametric bootstrap.
We simulated $B=100$ datasets from the all-shared SIRJPF2 maximum likelihood estimate and recomputed the MCAP profile and 95\% confidence interval, on each simulated dataset, for four target parameters: the native and invasive birth rates $r^{\native}$ and $r^{\invasive}$, the parasite spore process noise $\sigma_P$, and the food process noise $\sigma_F$.
Profiles were computed with the same iterated-filtering procedure used for the data analysis in the main paper (run-level matching the production runs).}

\subsection{Empirical coverage}

\bye{Table~\ref{Table:mcap-coverage} summarises the headline result.
For three of the four parameters the 95\% MCAP confidence interval covers the true value in the majority of simulations: 77.8\% for $r^{\native}$ ($77/99$; one of the $100$ profile runs did not complete, so its denominator is $99$), 87.0\% for $\sigma_P$ ($87/100$), and 74.0\% for $\sigma_F$ ($74/100$).
The Monte Carlo standard error of each coverage estimate, $\sqrt{\hat c(1-\hat c)/n}$, is about $4.2$, $3.4$ and $4.4$ percentage points respectively, so the shortfalls for $r^{\native}$ and $\sigma_F$ lie roughly four to five standard errors below the nominal $0.95$ and are not attributable to bootstrap noise.
For $r^{\invasive}$ the profile is essentially flat: in the SIRJPF2 process model $r^{\invasive}$ enters only through the product $r^{\invasive} f_S^{\invasive}$ in the invasive juvenile recruitment term, and $f_S^{\invasive}$ is itself only weakly constrained, so the level of $r^{\invasive}$ is not separately identifiable; the individual MCAP confidence intervals span the entire parameter grid and the nominal 95.8\% empirical coverage is degenerate rather than informative.
We therefore report coverage results only for the three identifiable parameters in the main response and explicitly document the invasive recruitment/infection ridge here: $r^{\invasive}$ and $p^{\invasive}$ are identified only in combination with $f_S^{\invasive}$, through the products $r^{\invasive} f_S^{\invasive}$ (the effective invasive birth rate) and $p^{\invasive} f_S^{\invasive}$ (the effective invasive infection rate) reported in the parameter table.}

\begin{table}[ht]
\centering
\caption{\label{Table:mcap-coverage}\bye{Empirical coverage of 95\% MCAP confidence intervals across $B=100$ bootstrap datasets simulated from the SIRJPF2 all-shared MLE.}}
\begin{tabular}{lccc}
\toprule
Parameter & Truth (log scale) & Mean MCAP MLE & Empirical coverage \\
\midrule
$r^{\native}$       &  3.71 &  4.36 (biased high)            & 77/99 = 77.8\% \\
$\sigma_P$          & $-1.30$ & $-1.73$ (biased low)          & 87/100 = 87.0\% \\
$\sigma_F$          & $-1.94$ & $-2.49$ (biased low)          & 74/100 = 74.0\% \\
$r^{\invasive}$     & 12.28 & profile flat over grid          & degenerate (dropped) \\
\bottomrule
\end{tabular}
\end{table}

\bye{The empirical undercoverage for $r^{\native}$, $\sigma_P$ and $\sigma_F$ is at first glance concerning, since profile likelihood methods are asymptotically justified by local asymptotic normality.}

\subsection{Diagnosing undercoverage with three log-likelihoods}

\bye{Before adding compute, we ran a three-log-likelihood diagnostic on five representative simulated datasets ($b \in \{2, 25, 50, 75, 95\}$).
On each dataset we evaluated three log-likelihoods: $\ell_{\text{truth}}$ at the true parameter $\theta_{\text{true}}$ via particle filtering with no further optimisation; $\ell_{\text{profile}}^{\max}$, the best log-likelihood across all profile points in the existing coverage profile; and $\ell_{\text{global}}^{\text{best}}$, the best log-likelihood from a 20-chain iterated-filtering run with all parameters free, warm-started from $\theta_{\text{true}}$.

Across all four target parameters and all five diagnostic datasets we find:
$\overline{\ell_{\text{profile}}^{\max} - \ell_{\text{truth}}} \approx 8.5$,
$\overline{\ell_{\text{global}}^{\text{best}} - \ell_{\text{truth}}} \approx 4.5$, and
$\overline{\ell_{\text{global}}^{\text{best}} - \ell_{\text{profile}}^{\max}} \approx -4$.
The first inequality says the profile run reaches a log-likelihood that is, on average, $8.5$ units above the true parameter value.
The second is the same comparison using the warm-started global search.
The third confirms that the production profile is more thorough than 20-chain warm-from-truth iterated filtering, and so the undercoverage is not caused by under-converged profiling.

The SIRJPF2 model has $p = 24$ free interior parameters (the parameter vector has 26 entries, with $\sigma_{S^{\native}}$ and $\sigma_{S^{\invasive}}$ fixed at the boundary value zero), fit to an $80$-observation panel ($U=8$ units, $N_u = 10$ time points each), so the asymptotic Wilks expectation for $\ell_{\text{MLE}} - \ell_{\text{truth}}$ is $p/2 = 12$.
The unrestricted gap $\overline{\ell_{\text{global}}^{\text{best}} - \ell_{\text{truth}}} \approx 4.5$ and the more-thorough profile gap $\approx 8.5$ are two searches of differing thoroughness targeting the same quantity $\ell_{\text{MLE}} - \ell_{\text{truth}}$ (the profile maximises over the $60$--$80$ separately optimised grid points and so climbs higher than the $20$-chain global search), and they bracket that gap below the heuristic $p/2 = 12$ asymptotic envelope, consistent with a weakly-identified model on a short panel whose estimator is displaced less than its full asymptotic amount along flat directions.
Both gaps are maxima---over Monte Carlo replication noise ($\approx 0.5$ log units) and, more substantially, over distinct optimisation chains and grid points---so they are upward-biased estimates of $\ell_{\text{MLE}} - \ell_{\text{truth}}$; MCAP corrects the Monte Carlo component when forming the interval.
On the datasets where every warm-started chain independently exceeds the truth the displacement is unambiguous, so the undercoverage reflects a genuine finite-sample displacement of the maximum likelihood estimate along weakly-identified ridges rather than a failure of the MCAP construction.}

\subsection{Identifiability findings}

\bye{In addition to the four target parameters, we examined the within-chain dispersion of all 24 free parameters across the 20 iterated-filtering chains used in the diagnostic study.
Three parameters showed within-chain standard deviations of $\approx 2.5$ log units each: $r^{\invasive}$, $f_S^{\invasive}$, and $p^{\invasive}$.
Inspecting the SIRJPF2 process model, $r^{\invasive}$ and $p^{\invasive}$ enter only in combination with $f_S^{\invasive}$---$r^{\invasive} f_S^{\invasive}$ in the invasive juvenile recruitment term and $p^{\invasive} f_S^{\invasive}$ in the invasive infection terms---while $f_S^{\invasive}$ appears on its own only in the (weakly informative) food and parasite depletion terms, so the levels of all three are individually non-identifiable along this ridge.
Two further parameters, $\sigma_{I^{\invasive}}$ and $\sigma_{I^{\native}}$, had within-chain standard deviations of $\approx 5$ log units each (a factor of $\approx 120$) and we treat them as essentially non-identifiable.
The better-identified parameters $r^{\native}$, $f_S^{\native}$, $p^{\native}$ and $\sigma_F$ had within-chain standard deviations of $1.4$--$1.5$ log units, with finite-sample log-MLE drift of order $\pm 1$, while $\sigma_P$ was intermediate ($\approx 2.3$).
Within-chain dispersion is thus an identifiability map rather than a direct predictor of coverage: it does not order monotonically with the empirical coverage reported above---$\sigma_F$, for instance, has the smallest dispersion yet the lowest coverage---because the coverage of each identifiable parameter reflects the interaction of its finite-sample bias with the width of its MCAP interval.}

\bye{
\begin{figure}[ht]
\centering
\begin{tabular}{cc}
\includegraphics[width=0.45\linewidth]{coverage/coverage_plot_rn.pdf} &
\includegraphics[width=0.45\linewidth]{coverage/coverage_plot_sigP.pdf} \\
(a) $r^{\native}$ & (b) $\sigma_P$ \\[2mm]
\includegraphics[width=0.45\linewidth]{coverage/coverage_plot_sigF.pdf} &
\includegraphics[width=0.45\linewidth]{coverage/coverage_plot_ri.pdf} \\
(c) $\sigma_F$ & (d) $r^{\invasive}$ (product ridge; dropped) \\
\end{tabular}
\caption{\label{fig:mcap-coverage-plots}Empirical coverage of MCAP 95\% confidence intervals across $B=100$ bootstrap datasets simulated from the SIRJPF2 all-shared MLE, for the four target parameters. In each panel, the horizontal lines mark each simulated dataset's MCAP confidence interval, ordered by midpoint; the vertical solid line marks the true parameter value; a CI ``covers'' if it intersects the true value. Panels~(a)--(c) show identifiable parameters with finite-sample undercoverage; for $r^{\native}$ this reflects displacement of the maximum likelihood estimate, whereas for $\sigma_F$, whose estimate is nearly unbiased, it reflects finite-sample miscalibration of the interval width (see Table~\ref{Table:three-loglik}). Panel~(d) shows the $r^{\invasive}$ profile, which is flat on the parameter axis because $r^{\invasive}$ enters the SIRJPF2 process model only through the recruitment product $r^{\invasive} f_S^{\invasive}$.}
\end{figure}

\begin{table}[ht]
\centering
\caption{\label{Table:three-loglik}Three-log-likelihood diagnostic for five representative simulated datasets. $\ell_{\text{truth}}$ is the particle-filter log-likelihood at the data-generating $\theta_{\text{true}}$. $\ell_{\text{prof}}^{\max}$ is the maximum across all profile points in the production coverage profiles (averaged across the four target parameters within each dataset). $\ell_{\text{glob}}^{\text{best}}$ is the best log-likelihood from a 20-chain iterated-filtering search warm-started at $\theta_{\text{true}}$ with all parameters free. Positive gap values indicate the corresponding estimator reached a higher likelihood than the truth on that dataset.}
\begin{tabular}{rrrrrrr}
\toprule
$b$ & $\ell_{\text{truth}}$ & $\ell_{\text{prof}}^{\max}$ & $\ell_{\text{glob}}^{\text{best}}$ & $\ell_{\text{prof}}^{\max} - \ell_{\text{truth}}$ & $\ell_{\text{glob}}^{\text{best}} - \ell_{\text{truth}}$ & $\ell_{\text{glob}}^{\text{best}} - \ell_{\text{prof}}^{\max}$ \\
\midrule
 2 & $-861.3$ & $-851.7$ & $-856.2$ &  $9.6$ &  $5.1$ & $-4.5$ \\
25 & $-932.1$ & $-925.0$ & $-928.6$ &  $7.2$ &  $3.5$ & $-3.6$ \\
50 & $-877.0$ & $-868.5$ & $-870.6$ &  $8.6$ &  $6.5$ & $-2.1$ \\
75 & $-876.1$ & $-870.5$ & $-876.2$ &  $5.6$ & $-0.1$ & $-5.7$ \\
95 & $-910.3$ & $-898.1$ & $-903.0$ & $12.1$ &  $7.4$ & $-4.9$ \\
\midrule
\textbf{mean} & & & & $\mathbf{8.6}$ & $\mathbf{4.5}$ & $\mathbf{-4.1}$ \\
\bottomrule
\end{tabular}
\end{table}}

\subsection{Coverage correction and interpretation}

\bye{We report the parametric-bootstrap coverage as our primary statement about the reliability of the MCAP intervals, rather than treating the undercoverage as a defect to be hidden. By ``exploratory rather than confirmatory'' we mean this operationally: the SIRJPF2 analysis is designed to characterise plausible mechanisms and to map which parameters the eight-unit panel identifies, not to deliver certified $95\%$-coverage guarantees for individual weakly-identified parameters for downstream decision use. For the well-identified parameters the bootstrap shows near-nominal coverage and the intervals can be read at face value; for the weakly-identified parameters the residual undercoverage is one further, quantified component of the uncertainty, to be weighed alongside structural and model uncertainty.

The same parametric bootstrap also lets us calibrate the interval when a nominal-coverage interval is wanted. The profile log-likelihood drop at the data-generating value, $D = \ell^{\max} - \ell(\theta_{\text{true}})$, exceeds the asymptotic half-deviance cutoff $\chi^2_{1,0.95}/2 = 1.92$ on more than $5\%$ of simulated datasets; replacing that cutoff by the $95$th percentile of the simulated $D$ restores $95\%$ coverage by construction. Table~\ref{Table:mcap-corrected} reports, for each parameter, the basic MCAP interval, its empirical coverage, and the corrected interval obtained with the calibrated cutoff.}

\begin{table}[ht]
\centering
\caption{\label{Table:mcap-corrected}\bye{Basic MCAP $95\%$ confidence intervals (log scale), their parametric-bootstrap coverage, the calibrated cutoff, and the cutoff-corrected intervals that target $95\%$ coverage. The basic intervals reproduce the values in Table~1 of the main text. Shown for the three identifiable target parameters whose coverage simulation is complete; the remaining parameters of interest follow once their simulations finish.}}
\begin{tabular}{lccccc}
\toprule
Parameter & Basic CI & Coverage & Cutoff & Corrected CI & Width \\
\midrule
$r^{\native}$ & $[3.27,\ 5.27]$   & $77.8\%$ & $5.10$ & $[2.80,\ 6.21]$   & $1.7\times$ \\
$\sigma_P$    & $[-3.12,\ -0.87]$ & $87.0\%$ & $3.34$ & $[-3.52,\ -0.63]$ & $1.3\times$ \\
$\sigma_F$    & $[-2.92,\ -1.46]$ & $74.0\%$ & $4.51$ & $[-3.97,\ -1.09]$ & $2.0\times$ \\
\bottomrule
\end{tabular}
\end{table}

\bye{Two qualifications attach to the corrected interval. First, the calibration is a parametric bootstrap from the fitted estimate $\hat\theta$: it corrects coverage of $\hat\theta$ under the fitted model, and so is reliable only insofar as $\hat\theta$ approximates the truth and the model is correctly specified---the assumption that is weakest for the weakly-identified parameters the correction most affects. Second, the calibrated cutoff is estimated from $B=100$ simulations and is itself noisy, and the in-sample $95\%$ is tautological. To check that the correction is not merely refitting its own simulations, we recalibrated the cutoff on a random half of the datasets and measured coverage on the held-out half: the out-of-sample coverage is $93.5\%$ (mean over $500$ random splits, standard deviation $\approx 5$ percentage points), close to nominal, indicating that the correction captures a genuine, generalisable feature of the finite-sample profile rather than overfitting. The practical content of Table~\ref{Table:mcap-corrected} is therefore that attaining nominal coverage in this model requires intervals roughly $1.3$ to $2$ times wider than the asymptotic ones. We report the basic MCAP interval as the primary, directly comparable estimate and the corrected interval as a transparent measure of the finite-sample cost of nominal coverage.}

\section{AIC Selection-Rate Simulation}\label{sec:aic-power}

\bye{Section stub created for the revision (Task~\#2), addressing the referee's request for a Stocks~(2020) Table~D3-style assessment. Computation owed: simulate panels under (a)~all-shared truth and (b)~one-parameter-unit-specific truth for SIRJPF2 (and ideally SRJF2 / Dent SIRJPF if compute allows); for each simulated panel refit both null and alternative models, compute AIC, and tabulate the proportion of simulations in which AIC selects the data-generating model. Suggested sample size: 50--100 simulated panels per truth scenario. Output target: a single table analogous to \citet{stocks20} Supplementary Table~D3 plus a one-paragraph narrative.}

\clearpage

\section{No-\texorpdfstring{$F$}{F} Ablation}\label{sec:no-F-ablation}

\bye{To assess whether the latent food compartment $F$ is a load-bearing component of the SIRJPF2 model rather than an incidental modeling choice, we fit an ablated model in which $F$ is held fixed at its initial value $F(t_0)=16.667$ for the entire trajectory.
Concretely, we remove the $F$ dynamics (its consumption losses, the renewal inflow, and the $F$ process noise $\sigma_F$) so that food no longer depletes as it is consumed by the Daphnia population; the birth terms still read $F$ through $r^{\species} f_S^{\species} F$, but because $F$ is now constant the per-capita recruitment rate no longer responds to a depletable resource.
The ablated model is fit to the observed data with exactly the same panel iterated-filtering protocol as the all-shared SIRJPF2 model (three rounds of \texttt{mif2}, $N_{\mathrm{mif}}=150/150/400$, $N_p=1000$ particles, geometric cooling with \texttt{cooling.fraction.50}~$=0.7$), warm-started from the all-shared maximum likelihood estimate.
Dropping $\sigma_F$ reduces the free-parameter count from $24$ to $23$ (the boundary-fixed $\sigma_{S^{\native}}=\sigma_{S^{\invasive}}=0$ remain excluded from both counts), so the ablation is favoured by the AIC complexity penalty; the two models are non-nested, since freezing the $F$ dynamics is a structural change rather than a parameter restriction, and we therefore compare them by AIC.}

\begin{table}[ht]
\centering
\caption{\label{Table:no-F-ablation}\bye{Comparison of the all-shared SIRJPF2 model against its no-$F$ ablation, in which the food compartment is held fixed at its initial value. Log-likelihoods are evaluated by particle filtering; the standard error shown for the ablation is the Monte Carlo standard error of the maximised log-likelihood across replicate searches. Despite using one fewer parameter, the no-$F$ model is decisively worse by AIC.}}
\begin{tabular}{lccc}
\toprule
Model & Free parameters $k$ & Max log-likelihood & AIC \\
\midrule
All-shared (depletable $F$) & $24$ & $-880.6$            & $1809.1$ \\
No-$F$ ($F$ held constant)  & $23$ & $-887.8\ (\pm 0.2)$ & $1821.6$ \\
\midrule
...
\bye{Freezing the food compartment costs approximately $7.2$ log-likelihood units relative to the all-shared model, even though the ablated model is granted a free parameter back; on the AIC scale the depletable-$F$ model is preferred by $\Delta\mathrm{AIC}\approx 12.4$, which is decisive on the conventional evidential scale ($\Delta\mathrm{AIC}>10$).
The mechanistic interpretation is direct.
With a non-depleting resource, recruitment into the juvenile classes $J^{\native}$ and $J^{\invasive}$ continues at an undiminished rate as the adult population grows, so the model cannot reproduce the resource-limited turnover that drives the observed rise-and-fall in Daphnia abundance.
The feedback in which a growing population draws down $F$, which in turn throttles recruitment, is precisely the mechanism that generates the peak-then-collapse signature in the data.
This ablation therefore confirms that explicit modeling of resource depletion, via the latent food compartment, is necessary rather than incidental, addressing the editor's question of whether the role of $F$ should be demonstrated explicitly.}
...
Empty cells in the bootstrap column correspond to comparisons for which the simulation study is still in progress.
...
\begin{tabular}{||c |c |c |c|c|c|c||}
 \hline
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(MPIF)} & \makecell[c]{AIC\\(MPIF)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\varnothing$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[1,]$AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[1,]$boot_lrt_pval)} \\
$\theta^{\species}_{I,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[2,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[2,]$boot_lrt_pval)} \\
$p^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[3,]$boot_lrt_pval)} \\
$\theta_{P,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[4,]$boot_lrt_pval)} \\
$\xi_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[5,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[5,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[5,]$boot_lrt_pval)} \\
$\theta^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[6,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[6,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[6,]$boot_lrt_pval)} \\
$f^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[7,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[7,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[7,]$boot_lrt_pval)} \\
$r^{\species}_{\unit}$ & \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_para_parameter_table[8,]$block_AIC,6)} & \Sexpr{format_pvalue(target_para_parameter_table[8,]$lrt_pval)} & \Sexpr{format_pvalue(target_para_parameter_table[8,]$boot_lrt_pval)} \\[1ex]
 \hline
\end{tabular}
\end{center}
...
\bye{%% PROVISIONAL --- estimated bootstrap $p$-values for the cells of
%% Table~\ref{Table:SIRJPF2_model_comparison} whose simulation is still in progress.
%% Computed so far (block PIF): $\xi$ ($B=98$), $\theta_S$ ($B=100$),
%% $\theta_P$ ($B=17$, partial). Remaining values marked [est] assume the
%% block-resampled bootstrap tracks the $\chi^2$ reference, as the three computed
%% cells do; replace with the completed $B=100$ values when available.
%% DECISION-CRITICAL: $\theta_I^{\species}$ and $p^{\species}$ --- if the block
%% bootstrap tracks $\chi^2$ they become significant ($\approx 0.05$), reversing the
%% earlier non-block result; the full computation must confirm.
\begin{table}[ht]
\centering
\caption{\label{Table:SIRJPF2_boot_provisional}\textbf{Provisional, estimated} block-resampled bootstrap LRT $p$-values for the SIRJPF2 unit-specific alternatives. Values marked [est] are estimates pending completion of the $B=100$ simulation; the rest are computed (with $B$ as noted in the text).}
\begin{tabular}{lccc}
\toprule
Unit-specific alternative & $\Lambda_{\mathrm{obs}}$ & $p_{\chi^2}$ & $p_{\mathrm{boot}}$ \\
\midrule
$\xi_{\unit}$                          & $-0.55$ & $1.00$  & $0.98$ \\
$\theta^{\species}_{S,\unit}$          & $-1.06$ & $1.00$  & $1.00$ \\
$\theta_{P,\unit}$                     & $4.48$  & $0.72$  & $0.70$~[est] \\
$\theta^{\species}_{I,\unit}$          & $25.6$  & $0.029$ & $0.05$~[est, critical] \\
$p^{\species}_{\unit}$                 & $25.0$  & $0.034$ & $0.06$~[est, critical] \\
$r^{\species}_{\unit}$                 & $-6.75$ & $1.00$  & $0.99$~[est] \\
$f^{\species}_{S,\unit}$               & $-1.93$ & $1.00$  & $0.98$~[est] \\
\bottomrule
\end{tabular}
\end{table}}
...
\begin{tabular}{||c |c |c |c|c|c|c||}
 \hline
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(block)} & \makecell[c]{AIC\\(block)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\theta^{\native}_{I,\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[1,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[1,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[1,]$boot_lrt_pval)} \\
$\varnothing$  & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[2,]$AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[2,]$boot_lrt_pval)} \\
 $r^{\native}_{\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[3,]$boot_lrt_pval)} \\
$p^{\native}_{\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[4,]$boot_lrt_pval)} \\
$\xi_{\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[5,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[5,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[5,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[5,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[5,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[5,]$boot_lrt_pval)} \\
$\theta^{\native}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[6,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[6,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[6,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[6,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[6,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[6,]$boot_lrt_pval)} \\
$f^{\native}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[7,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[7,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[7,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[7,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[7,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[7,]$boot_lrt_pval)} \\
$\theta_{P,\unit}$& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[8,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[8,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[8,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_para_parameter_table[8,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_para_parameter_table[8,]$lrt_pval)} & \Sexpr{format_pvalue(dent_para_parameter_table[8,]$boot_lrt_pval)} \\[1ex]
 \hline
\end{tabular}
\end{center}
...
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(block)} & \makecell[c]{AIC\\(block)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\xi_{\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[1,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[1,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[1,]$boot_lrt_pval)} \\
$\varnothing$  & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[2,]$AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[2,]$boot_lrt_pval)} \\
$p^{\invasive}_{\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[3,]$boot_lrt_pval)} \\
$\theta^{\invasive}_{I,\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[4,]$boot_lrt_pval)} \\
$f^{\invasive}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[5,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[5,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[5,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[5,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[5,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[5,]$boot_lrt_pval)} \\
$\theta_{P,\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[6,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[6,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[6,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[6,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[6,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[6,]$boot_lrt_pval)} \\
$r^{\invasive}_{\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[7,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[7,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[7,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[7,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[7,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[7,]$boot_lrt_pval)} \\
$\theta^{\invasive}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[8,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[8,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[8,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_para_parameter_table[8,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_para_parameter_table[8,]$lrt_pval)} & \Sexpr{format_pvalue(lum_para_parameter_table[8,]$boot_lrt_pval)} \\[1ex]
 \hline
\end{tabular}
\end{center}
...
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(block)} & \makecell[c]{AIC\\(block)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\varnothing$  & \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[1,]$AIC,6)} & \Sexpr{format_pvalue(target_no_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(target_no_para_parameter_table[1,]$boot_lrt_pval)} \\
$\theta^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[2,]$block_AIC,6)} & \Sexpr{format_pvalue(target_no_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(target_no_para_parameter_table[2,]$boot_lrt_pval)} \\
$f^{\species}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(target_no_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(target_no_para_parameter_table[3,]$boot_lrt_pval)} \\
$r^{\species}_{\unit}$  & \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(target_no_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(target_no_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(target_no_para_parameter_table[4,]$boot_lrt_pval)} \\[1ex]
 \hline
\end{tabular}
\end{center}
...
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(block)} & \makecell[c]{AIC\\(block)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\varnothing$ & \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[1,]$AIC,6)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[1,]$boot_lrt_pval)} \\
$\theta^{\native}_{S,\unit}$  & \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[2,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[2,]$boot_lrt_pval)} \\
$r^{\native}_{\unit}$ & \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[3,]$boot_lrt_pval)} \\
$f^{\native}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(dent_no_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(dent_no_para_parameter_table[4,]$boot_lrt_pval)}\\[1ex]
 \hline
\end{tabular}
\end{center}
...
Specific parameters  & Max log-likelihood & AIC & \makecell[c]{Max log-likelihood\\(block)} & \makecell[c]{AIC\\(block)} & \makecell[c]{LRT\\$p$-value\\($\chi^2$)} & \makecell[c]{LRT\\$p$-value\\(bootstrap)}\\ [0.5ex]
 \hline\hline
$\varnothing$ & \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[1,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[1,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[1,]$AIC,6)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[1,]$lrt_pval)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[1,]$boot_lrt_pval)} \\
$\theta^{\invasive}_{S,\unit}$  & \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[2,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[2,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[2,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[2,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[2,]$lrt_pval)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[2,]$boot_lrt_pval)} \\
$r^{\invasive}_{\unit}$ & \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[3,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[3,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[3,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[3,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[3,]$lrt_pval)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[3,]$boot_lrt_pval)} \\
$f^{\invasive}_{S,\unit}$ & \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[4,]$ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[4,]$AIC,6)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[4,]$block_ll,5)}& \Sexpr{mysignif_ll_AIC(lum_no_para_parameter_table[4,]$block_AIC,6)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[4,]$lrt_pval)} & \Sexpr{format_pvalue(lum_no_para_parameter_table[4,]$boot_lrt_pval)} \\[1ex]
 \hline
\end{tabular}
\end{center}
