...
alt_names <- c("xi", "theta_Si", "theta_Ii", "theta_P", "probi", "ri", "f_Si")
...
# ---- Load observed log-likelihoods from the Lum SIRJPF parameter table ----
load("../../../../data/Simple_dynamics/Lum/para/Lum_para_loglik_df.rds")
null_ll_obs  <- lum_para_parameter_table["all_shared", "ll"]
null_AIC_obs <- lum_para_parameter_table["all_shared", "AIC"]
...
# Row name mapping: alt_name matches row name in parameter table
row_map <- list(
  xi       = "xi",
  theta_Si = "theta_Si",
  theta_Ii = "theta_Ii",
...
for (alt in alt_names) {
  ...
  if (B_valid == 0) {
    ...
    boot_mean <- mean(Lambda_boot)
    boot_sd <- sd(Lambda_boot)
  }
  cat(sprintf("  p_chisq = %.4f   p_boot = %s   (B = %d)\n",
              p_chisq, ifelse(is.na(p_boot), "NA", sprintf("%.4f", p_boot)), B_valid))
  ...
  results <- rbind(results, data.frame(
    ...
    p_boot       = p_boot,
    B_completed  = B_valid,
    Lambda_boot_mean = boot_mean,
    Lambda_boot_sd   = boot_sd,
    stringsAsFactors = FALSE
  ))
  ...
  if (B_valid > 10) {
    p <- ggplot(data.frame(Lambda = Lambda_boot), aes(x = Lambda)) +
      ...
      labs(title = paste0("Lum SIRJPF bootstrap LRT: ", alt, " (df=", df, ")"),
